...
| File | Verdict | Reason |
|---|---|---|
| `archive/migrations-applied/sql/003_notes_todos_reminders.sql` | HISTORICAL | Notes/todos schema; one-time DDL. |
| `archive/migrations-applied/sql/004_production_scheduling.sql` | HISTORICAL | Production scheduling tables; one-time DDL (also mirrored inline in `main.py`). |
| `archive/migrations-applied/sql/005_customer_aliases.sql` | HISTORICAL | Customer aliases table; one-time DDL (mirrored inline). |
| `archive/migrations-applied/sql/012_pg_trgm_product_search.sql` | HISTORICAL | Enables `pg_trgm`; one-time. |
| `archive/migrations-applied/sql/013_shipment_tables.sql` | HISTORICAL | Shipments / shipment_lines DDL; one-time. |
| `archive/migrations-applied/sql/014_fix_so260217001_flake_overshipment.sql` | HISTORICAL | One-time data fix. |
| `archive/migrations-applied/sql/015_fix_lot131_negative_balance.sql` | HISTORICAL | One-time adjustment. |
| `archive/migrations-applied/sql/016_backfill_received_at.sql` | HISTORICAL | One-time backfill. |
| `archive/migrations-applied/sql/017_transaction_status_and_void_ghosts.sql` | HISTORICAL | One-time DDL + void cleanup. |
| `archive/migrations-applied/sql/018_backfill_pre_migration_shipments.sql` | HISTORICAL | One-time backfill. |
| `archive/migrations-applied/sql/019_populate_case_size_lb.sql` | HISTORICAL | One-time backfill. |
| `archive/migrations-applied/sql/020_numeric_precision.sql` | HISTORICAL | One-time column-type change. |
| `archive/migrations-applied/sql/021_fix_so260217008_undershipment.sql` | HISTORICAL | One-time data fix. |
| `archive/migrations-applied/sql/022_close_so260217001.sql` | HISTORICAL | One-time order close. |
| `archive/migrations-applied/sql/023_reconcile_so260312005_dicarlo.sql` | HISTORICAL | One-time reconciliation. |
| `archive/migrations-applied/sql/024_close_so260213001_juliette.sql` | HISTORICAL | One-time order close. |
| `archive/migrations-applied/sql/025_set_supplier_lot_sprinkles_26-03-10-FOUN-001.sql` | HISTORICAL | One-time supplier-lot set. |
| `archive/migrations-applied/sql/026_add_bs_8oz_granola_products.sql` | HISTORICAL | One-time product insert. |
| `archive/migrations-applied/sql/027_add_sliced_almonds_products.sql` | HISTORICAL | One-time product insert. |
| `archive/migrations-applied/sql/028_add_is_service_to_products.sql` | HISTORICAL | One-time DDL. |
| `archive/migrations-applied/sql/029_rename_unknown_lot_to_25216.sql` | HISTORICAL | One-time rename. |
| `archive/migrations-applied/sql/030_backfill_standalone_shipment_records.sql` | HISTORICAL | One-time backfill. |
| `archive/migrations-applied/sql/031_relax_quantity_lb_check_for_service_items.sql` | HISTORICAL | One-time CHECK constraint relax. |
| `archive/migrations-applied/sql/032_backfill_skus_and_merge_bs_cocoa.sql` | HISTORICAL | One-time SKU backfill + product merge. |
| `archive/migrations-applied/sql/033_force_close_so260326002_ace_endico.sql` | HISTORICAL | One-time order close. |
| `archive/migrations-applied/sql/034_force_close_so260414003_hannas.sql` | HISTORICAL | One-time order close. |
| `archive/migrations-applied/sql/034a_fruit_nut_bom_bake_line_copack.sql` | HISTORICAL | Backfilled direct-SQL session (2026-05-18); applied. |
| `archive/migrations-applied/sql/036_drop_bake_line.sql` | HISTORICAL | One-time DDL drop (2026-05-19); resolved 035 slot conflict per commit 1a74969. |
...
| File | Verdict | Reason |
|---|---|---|
| `audits/reports/AUDIT_GPT_FABRICATION_2026-04-21.md` | **AUDIT** | Dated GPT fabrication audit. |
| `audits/reports/TRACEABILITY_AUDIT_2026-03-24.md` | **AUDIT** | Dated traceability audit. |
| `audits/reports/DASHBOARD_ACTIONABILITY_AUDIT.md` | **AUDIT** | Dashboard actionability audit (undated but audit-shaped). |
...
| `archive/superseded-instructions/GUIDE.md` | **SUPERSEDED** | Header is `Dummy Guide (v2.5.0)`. Live API is 3.4.0; this is a v2.5.0-era doc. |
| `archive/superseded-instructions/SALES_API.md` | **SUPERSEDED** | Header is `Sales API Reference (v2.5.0)`. Live API is 3.4.0; references endpoints/shapes from the 2.x series. |
| `archive/migrations-applied/factory_ledger_reconciliation.sql` | **HISTORICAL** | One-time reconciliation report SQL, run-in-editor only; not referenced by code. |
...
# `git mv` plan (executed 2026-05-19)

Executed in four sequential commits on `claude/serene-panini-dc449d`:
`947fdd5` (schemas) → `cc0b380` (migrations) → `d22977e` (audits) → `ba5baab` (instructions).
The plan as originally proposed is preserved below for the record.

---

# Proposed `git mv` plan (executed)
