...
## 2026-05-28 18:05 — Floor GPT: add addFoundInventory op; fix rename, supplier-lot, found-inventory routing
- **File(s) changed:** `gpt-configs/schemas/openapi-floor.yaml`, `gpt-configs/sources/floor-specific.md`, `gpt-configs/dist/GPT_FLOOR_INSTRUCTIONS.md`
- **What changed:** (1) Added `POST /inventory/found` as operationId `addFoundInventory` plus its `AddFoundInventoryRequest` schema (fields mirror the Pydantic model in main.py:810). 200 response documents lot_id/lot_code/product_name/quantity/uom/entry_source and explicitly notes NO transaction_id is returned. Floor schema now 22 operations. (2) ROUTING — FLOOR rename rule rewritten as two-step: lot code + "rename"/"change to" → getLotByCode for numeric lot_id → renameLot (renameLot takes lot_id, not lot_code). (3) SUPPLIER LOT CROSS-REF lookup split: quick "do we already have supplier lot X?" → getLotsBySupplierLot (lightweight); full FDA recall trace → traceSupplierLot. (4) FOUND INVENTORY section corrected: removed the nonexistent supplier_lot_code:"UNKNOWN" claim, added product_id resolution via searchProducts/inventoryLookup (integer id, not name), reason_code + notes requirements, commits-immediately/no-preview note, and quote lot_code as the receipt. Rebuilt dist (7,219 chars, under 8,000 limit).
- **Why:** The Floor GPT had no exposed operation for found inventory despite the instructions referencing it; the rename routing implied passing a lot_code into renameLot (which takes lot_id); supplier-lot lookups all routed to the heavy recall trace; and the FOUND INVENTORY guidance described a field that does not exist on the endpoint and omitted how to obtain product_id.
