...
- **What changed:** Five items from Codex's REQUEST CHANGES on PR #49. **R1** — the actor cache did not deliver the 60 s revocation bound it documented: the freshness stamp was taken after the load rather than before it, an older overlapping refresh could overwrite a newer snapshot, and a snapshot already older than the TTL could be served. Refreshes now stamp the START time, carry a monotonic generation number and only publish if theirs is the newest, and never return a snapshot past its TTL — after `ACTOR_CACHE_REFRESH_ATTEMPTS` (3) the request fails every actor key CLOSED rather than authenticate against stale rows. **R2** — `last_used_at`'s 10-minute throttle was enforced only in an in-process map, which is per worker; the interval is now a predicate in the UPDATE's own `WHERE` clause, with the in-memory map kept as a shortcut so the hot path still costs no query. The write is isolated in `_write_actor_last_used()` and still cannot fail a request. **R5** — the packing slip's `?key=` query parameter reached the Uvicorn access log in plaintext, including on rejection; a `RedactKeyQueryParam` logging filter on `uvicorn.access`/`uvicorn.error` now replaces the parameter's value with `[REDACTED]`, leaving path, method, status and other parameters intact. **R3** — `update_order_line`'s changed `released_by` attribution was ruled INTENTIONAL by the owner and kept: `'legacy-shared-key'` was a placeholder and fixing that writer was a logged follow-up, so the three-key matrix now asserts actor name / `'dashboard'` / NULL at both its expiry and its shrink write site. **R4** — test gaps closed (below).
- **Validation:** 955 Python tests pass (906 before this pass + 49 new). New coverage: four cache-revocation tests, three of which fail against the pre-fix implementation and a fourth pinning that a load which FAILS slowly is not retried into a multi-minute hang; a two-real-connection test proving exactly one `last_used_at` write per key per 10 minutes (both connections write if the SQL predicate is removed); failure injection at the throttle write proving a read and an administrative exit still return 200; the three-key matrix on the legacy `PATCH .../status` exits for `cancelled` and `invoiced`, asserting the persisted `(status, state)` pair, `state_changed_by` and released rows' `released_by`; actor-key rejection on `POST .../lines` and `PATCH .../lines/{id}/cancel` with the rows checked untouched; an exhaustive walk of all 22 operations in `gpt-configs/schemas/openapi-floor.yaml` asserting `DASHBOARD_KEY_ALLOWLIST` decides each one (13 shared → accepted, 9 floor-exclusive → 403); and access-log redaction. `openapi-gpt-v3.yaml` untouched, still exactly 30 operations. Lock sequences re-extracted mechanically against `origin/main` (`72b5546`) and diffed: **identical**, all eleven write paths — no lock added, removed, or reordered.
- **Why:** "Deactivation takes effect within 60 seconds" and "one `last_used_at` write per key per 10 minutes" were both documented guarantees that the code did not actually provide, and a credential in the access log turns log access into key material.
...
- **What changed:** Branch `feat/so-health-strings` off `main` at `4a2ac50`. Code, tests and docs only — no migration, no `dashboard/` change. Three defects, all in the wording of `compute_so_health()`, none in the level: the tiers and every tier rule are untouched.
  **(1) `info` aggregated to one entry per order.** Unallocated pounds were reported one line at a time; a twelve-line order produced twelve near-identical sentences and the total appeared in none of them. Now one entry: `24,000 lb not allocated across 5 lines`, with `across N lines` **omitted when N is 1** — there is no count worth stating and nothing to disambiguate, so the single line names itself instead: `60 lb not allocated on Granola SS Chocolate Chip (70003)`. Lines whose unallocated pounds fall under `BALANCE_EPSILON` are not lines: a rounding crumb must not inflate the count nor turn a one-line order into a two-line one. When `ALLOCATIONS_ENFORCED` is off, ` (allocations not enforced)` is appended **once**, to that one entry — STATUS-011, a caveat is stated once at first relevance; repeated down a list it stops being read, including the time it mattered. The flag changes the wording and nothing else: the level never moves and `info_detail` is byte-identical on and off, asserted as a pair so the suffix cannot drift into carrying meaning.
  **(2) New `health.info_detail`.** Aggregation moves the per-line facts rather than losing them: a list of `{line_id, sku, product_name, unallocated_lb}`, one row per line with unallocated pounds, for the popover to expand the sentence back into. **Always present** (`[]` when there is nothing to expand, including on `closed`/`cancelled`), so a caller reads the key without checking for it; `unallocated_lb` is a **raw number**, not a formatted string, because a string cannot be summed, sorted or re-rounded — its entries sum to exactly the pounds in the sentence and the front end runs them through its own STATUS-006 formatter; `sku` and `product_name` are separate fields, not the joined label, so the popover can column them. `{level, reasons, info}` is otherwise unchanged and so is every caller.
  **(3) One number formatter — `_fmt_number()`, per FL-Design-Standards-MASTER STATUS-006.** Thousands separators always, pounds to whole numbers, never a trailing decimal: `13,500 lb`, not `13500.0000`, not `13500 lb`, not `13,500.00 lb`. **Half rounds UP via `Decimal(ROUND_HALF_UP)` — deliberately not Python's own formatting**: `f"{607.5:,.0f}"` is `608` but `f"{606.5:,.0f}"` is `606`, because `format` rounds half to *even*, so the same shortage prints larger or smaller depending on the digit before it, and a shortage that prints smaller than it is reads as less urgent than it is. Counts go through the same function — `on 2 lines`, `across 5 lines`, `1,200 days overdue` — not because a line count needs a separator today, but so nothing in a health string is hand-formatted and there is no second place for a format to drift. `{total_short:g}` and `{unallocated:g}` are gone; day counts are formatted in `_so_ship_phrase()`.
  **(4) New `_so_line_label()` — `product name (SKU)`.** `Granola SS Chocolate Chip (70003)` wherever a line is named in a reason or an info string: a bare SKU is a lookup the reader has to do, a bare name does not say which pack size. **It costs no query** — `SALES_ORDER_READINESS_SQL` already selects `p.name AS product` and `p.odoo_code AS sku` on every line row and `_load_sales_order_readiness()` already carries both into the dict health reads, so the name was being fetched and discarded. Degrades rather than printing an artefact: name only, code only, or the literal `line` when there is neither — never `Granola Maple ()`, never a bare `()`.
  **Docs:** new **"Step 2d — Health strings"** section in `docs/design/so-state-model-findings.md` — the aggregation table, `info_detail`'s three guaranteed properties, the STATUS-006 boundary table with `606.5` called out as the half-even control, the no-extra-query note on the product label, what did not change, and the mutation results.
  **Tests:** 231 in `tests/test_sales_order_state_model.py` (was 192), suite **837 passed, 0 failed** (was 798). Aggregation at 1 / 2 / 5 lines plus a sub-epsilon line that must not be counted, and an end-to-end pair through the endpoint; the enforcement suffix on and off stated as a pair whose only difference is the suffix, plus proofs that it never raises the level and never touches `info_detail`; `info_detail` one-row-per-line, summing to the sentence, always present, flag-independent; a thirteen-row `_fmt_number()` boundary table (999 / 1,000 / 999.5 / 607.5 / **606.5** / 12,345.4 / 12,345.5 / 13500.0000 / None) plus the STATUS-006 audit itself run against the assembled strings — no three-decimal number, no value of 1,000 or more without a separator; a seven-row `_so_line_label()` table covering both halves, either half missing, whitespace and neither; and the product-name substitution end-to-end. Existing string assertions updated (`Short 1400 lb` → `Short 1,400 lb`; the three whole-dict `{level, reasons, info}` comparisons now carry `info_detail`).
  **Mutation-verified and reverted, every new rule:** `ROUND_HALF_UP` → default half-even **2 fail** (`_fmt_number[606.5-607]` and `[0.5-1]`; the 607.5 row still passes, which is what makes it the control) · drop the thousands separator **16 fail** · `_so_line_label()` returns the bare SKU **7 fail** · un-aggregate to one `info` entry per line **6 fail** · drop the not-enforced note **3 fail** · `across N lines` boundary `> 1` → `>= 1` **6 fail** · drop `info_detail` from the returned shape **10 fail**.
- **Why:** The v2.1 tiers were right and the sentences were not. A partially-allocated order repeated itself down the popover without ever stating its total; numbers reached the screen in their stored form (`13500.0000`, `1400 lb`), which STATUS-006 exists to stop because re-reading a quantity is how the wrong quantity gets shipped; and lines were identified by a code the reader has to go look up.

---

## 2026-09-10 21:25 — Sales-order health v2.1: factory-local dates, a warning window, reason suppression
- **File(s) changed:** `main.py`, `tests/test_sales_order_state_model.py`, `docs/design/so-state-model-findings.md`, `FACTORY_LEDGER_CHANGELOG.md`, `CHANGE_LOG.md`
- **What changed:** Branch `feat/so-health-v2-1` off `main` at `7613af3`. Code, tests and docs only — no migration, no `dashboard/` change. `compute_so_health()` relabelled **"v2.1 — time-aware. Shape is the contract."**; the `{level, reasons, info}` shape (owner ruling 4) and every caller are untouched, and it still issues no SQL of its own.
  **(1) Factory-local dates.** New `FACTORY_TZ` env var (default `America/New_York`) with `_factory_tz()` / `_factory_today()` helpers, read on **every call** like `SO_HEALTH_CRITICAL_DAYS`; an unknown zone (`KeyError`/`ZoneInfoNotFoundError`) or a malformed one (`ValueError`) falls back to the default rather than raising, because this is a read path. Four sites moved off the server's clock: `compute_so_health()`'s default `today`, the `/sales/orders/counts` **overdue** bucket, each order's `overdue` field in `GET /sales/orders`, and the `?overdue_only=true` filter — which was SQL `CURRENT_DATE` and is now a parameterised `_factory_today()`, so the filter and the field in the same response can no longer disagree. Kept deliberately separate from module-level `PLANT_TIMEZONE` (same value today) so retuning the sales-order health clock cannot silently retune the ledger, shipment and production paths. Out of scope and unchanged: expected receipts (already on `get_plant_now()`), `GET /sales/dashboard`, the production planner's demand horizon.
  **(2) Three shortage windows.** New env var **`SO_HEALTH_WARNING_DAYS`, default 10**, alongside the existing `SO_HEALTH_CRITICAL_DAYS` (default 5); both now read through one shared `_so_health_window_days(name, default)` with the same non-negative-integer-or-fall-back rule. A shortage is **critical** inside the critical window (or past due), **warning** inside the warning window, and **`info` only** beyond it — `Short 1400 lb — ships in 13 days` — affecting no tier, because on a make-to-order book an unproduced order with a month of runway is the normal state of the work, not a problem. `critical` is additionally gated on the shortage having earned a place in `reasons`, so inverted windows produce a quiet order with an `info` line rather than a red badge with nothing to act on. Unchanged: overdue-with-stock-on-hand stays `warning`; Not-Ready within 2 days stays `warning`; a shortage with no ship date stays `warning`.
  **(3) Reason suppression.** The **"Not Ready to Ship"** reason is now suppressed whenever the same order is short — same `not short_lines` guard, and for the same reason, as the overdue-with-stock reason since v2: an order that cannot ship for want of material is not also news for want of a flag, and two reasons read as two problems. With the stock on hand the flag IS the blocker, so there it is kept. Suppressed means gone, not demoted — it does not reappear in `info`.
  **Docs:** the `so-state-model-findings.md` Health section rewritten v2 → **v2.1** with the three-window tier table, a site-by-site table of what the clock governs (and what is deliberately out of scope), all three env vars, the wording rules, and the mutation results.
  **Tests:** 192 in `tests/test_sales_order_state_model.py` (was 152) — a 29-row tier matrix (rows may now assert `info` as well as `reasons`) covering **both sides of all three boundaries**: `today + 5` critical vs `today + 6` warning, `today + 10` warning vs `today + 11` info, `today + 2` Not-Ready warning vs `today + 3` quiet. New clock tests pin an *instant* via a `_freeze()` helper that stands the process up as a UTC server (`datetime.now(tz)` honours the zone handed to it, `date.today()` returns the UTC day): an order due Sep 10 evaluated at **20:00 Eastern is not overdue** and reads `ships today`, and the same order at **00:30 Eastern** next morning is `1 day overdue — stock on hand`. Plus: both windows counted from the factory date, `FACTORY_TZ` moving the calendar (one instant, New York vs Los Angeles, two different "today"s) and falling back on garbage; the suppression rule stated as a pair so it cannot be half-reverted, unit and end-to-end; `SO_HEALTH_WARNING_DAYS` widened / narrowed / defaulted / fed garbage / proven independent of the critical window; and the counts bucket, the order's health badge, the list's `overdue` field and the `?overdue_only=true` filter all checked at both frozen instants. Suite **798 passed, 0 failed** (was 758).
  **Mutation-verified and reverted, all four changes:** `_factory_today()` → `date.today()` in `compute_so_health()` fails 5 tests including `test_an_order_due_today_is_not_overdue_at_8pm_eastern` (the 00:30 test still passes — at that instant both clocks agree, which is what makes it the control); the same revert in the counts endpoint fails both counts-clock tests; dropping the `not short_lines` suppression guard fails 6 (3 matrix rows + 3 suppression tests); the warning window's `<=` → `<` fails `shortage-exactly-the-warning-boundary` and the narrowed-env-var test.
- **Why:** At 8 PM Eastern the API called an order due **today** "1 day overdue", because Railway runs UTC and the UTC calendar has already turned over — on exactly the shift most likely to be reading the board. Separately, v2 fixed v1's flat-red board but replaced it with a flat-yellow one: every unproduced order carried a warning badge no matter how far out it shipped, which on a make-to-order book is every order that has not been made yet. And a short order was printing two reasons ("Short 500 lb" and "Not Ready to Ship") for what is one problem with one fix.

---

## 2026-09-11 01:30 — Sales-order health v2 (time-aware tiers) + "Ready to Ship" relabel
- **File(s) changed:** `main.py`, `tests/test_sales_order_state_model.py`, `docs/design/FL-Design-Standards-MASTER.md`, `docs/design/so-state-model-findings.md`, `FACTORY_LEDGER_CHANGELOG.md`, `CHANGE_LOG.md`
- **What changed:** Branch `feat/so-health-v2` off `main` at `e8a8aa2`. Code and docs only — no migration, no `dashboard/` change.
  **(1) `compute_so_health()` v1 → v2, time-aware.** Relabelled **"v2 — time-aware. Shape is the contract."** The `{level, reasons, info}` shape (owner ruling 4) and every caller are untouched, and it still issues no SQL of its own — it reads the same `SALES_ORDER_READINESS_SQL` output. Evaluated on **open orders only**:
  · **critical** — a stock shortage on a non-cancelled line AND `ship_by <= today + SO_HEALTH_CRITICAL_DAYS`. A past `ship_by` is inside that window by definition, so the rule's two halves are one comparison.
  · **warning** — a shortage further out than the window, or with no ship date at all; OR overdue (`ship_by < today`, `fulfillment != 'shipped'`) with **no** shortage; OR Ready to Ship unset with `ship_by <= today + 2`.
  · **info** — `unallocated_need_lb > 0`. Never affects the level.
  · **quiet** — nothing applies, or the order is closed/cancelled, which returns no reasons and no info at all.
  Highest tier wins; every applicable reason is listed. Reasons carry the specifics AND the date — `Short 735 lb on 2 lines — ships in 3 days`, `Short 500 lb — ships in 14 days`, `12 days overdue — stock on hand`, `Not Ready to Ship — ships tomorrow` — with the time half from one helper, `_so_ship_phrase()`. Shortages are aggregated into **one** reason (v1 emitted one per line, which buried both the total and the date); `on N lines` is omitted when N is 1. The overdue reason is guarded on there being no shortage, because the shortage reason already carries the overdue phrase.
  **(2) New env var `SO_HEALTH_CRITICAL_DAYS`, default 5.** Read on every call, the way `ALLOCATIONS_ENFORCED` and `FACTORY_READY_REQUIRED` are, so the window is retunable without a deploy. A non-integer or negative value falls back to 5 rather than raising — this is a read path and an env-var typo must not 500 the Sales Orders board. The Ready-to-Ship window is a separate constant `SO_HEALTH_READY_DAYS = 2`, deliberately **not** tied to the env var: the floor's flag is a same-week concern, not a supply one.
  **(3) `info` no longer gated on the allocation flag.** v1 suppressed the whole unallocated-pounds observation while `ALLOCATIONS_ENFORCED` was off. The pounds are unallocated either way; the flag only decides whether that blocks a shipment — so the observation is now unconditional and the `; allocations not enforced` note is appended only when the flag is off.
  **(4) Docs — "Ready to Ship" relabel.** `FL-Design-Standards-MASTER.md` bumped **1.1 → 1.2** with a new Appendix E. The Readiness dimension now reads **"Ready to Ship — the floor's flag, currently stored as Factory Ready"** in the STATUS-001 rule line, in the STATUS-001 worked example, and in the STATUS-012 Factory Ledger line. Rule IDs are unchanged and no rule was added, renumbered or removed. The same relabel lands on owner ruling 5 in `so-state-model-findings.md`, which gains a **Health v2** section carrying the tier table, the env var, the reason wording and what deliberately did not change; ruling 4 now records that the shape survived the rewrite and only the version number moved.
- **Mutation-verified, reverted:** changing the critical-window `<=` to `<` fails exactly two tests — `test_health_tier_matrix[shortage-exactly-the-boundary]` and `test_health_critical_window_narrows_with_the_env_var` — and nothing else. `git diff` does not carry the mutation.
- **Test count:** 758 passed, 0 failed (was 722). 152 tests in `tests/test_sales_order_state_model.py`, up from 116. Run in three foreground chunks (231 / 247 / 280) and once whole. The new tests are a **24-row tier matrix** driven straight against `compute_so_health()` with a pinned `today` — every row of the table above and **both sides of each boundary** (`today + 5` critical vs `today + 6` warning; `today` critical; `today - 1` critical; `today + 2` Not-Ready warning vs `today + 3` quiet) — plus multi-reason orders, closed/cancelled quiet, and the env var widened, narrowed, defaulted and fed garbage. The matrix is a unit test on purpose: the tiers are a pure function of (shortage, ship date, readiness flag, fulfillment), and proving them through Postgres would cost 24 seeded orders to test arithmetic no database is involved in. The DB-backed tests kept beside it prove the endpoint is **wired** to the function. All existing shape tests kept.
- **Why:** v1 made any stock shortage critical regardless of when the order shipped. The same 500 lb short is a different fact at three days out than at three months, so the red tier carried no information and the board read as a list of orders rather than a queue of work — the operator still had to open every red row to find this week's. Separately, "Factory Ready" is a storage detail (`sales_order_flags.ready`, migration 037) that had leaked into the operator's vocabulary in the written standards, while the counts endpoint already used the floor's own words (`ready_to_ship`, owner ruling 5).

---

## 2026-09-11 00:40 — Startup guard: Codex review fix pass (2 P2s — advisory lock, marker-failure fallback)
- **File(s) changed:** `main.py`, `tests/test_startup_migrations.py`, `FACTORY_LEDGER_CHANGELOG.md`, `CHANGE_LOG.md`
- **What changed:** Both P2s from the Codex review of PR #41 (MERGE AFTER FIXES), on the startup run-once guard only. Attribution (item 2) and the test-rot fix (item 3) passed review and are untouched.
  **(P2-1) The check-DDL-insert sequence is now atomic, not merely idempotent.** Every startup-migration transaction — the one that creates `migration_markers` included — opens with `pg_advisory_xact_lock(STARTUP_MIGRATION_LOCK_KEY)`, taken BEFORE the marker SELECT. The key is `216448987635338620` = the first 8 bytes of `sha256(b"factory_ledger.startup_migrations")` read as a big-endian signed int64, documented next to the helper with the one-liner that reproduces it and a NEVER-CHANGE-IT note (advisory locks are keyed by value alone, so two instances on different keys do not exclude each other). Transaction-scoped on purpose: it releases on COMMIT *or* ROLLBACK, so a crashed boot cannot hold it and no path needs an explicit unlock. The lock also covers `CREATE TABLE IF NOT EXISTS migration_markers`, which is itself a documented Postgres race between simultaneous creators. `IF NOT EXISTS` and `ON CONFLICT DO NOTHING` stay underneath as the second line of defence for the ungated fallback, which takes no lock.
  **(P2-2) A marker the app cannot read or write falls back to ungated — never to skipped.** If the marker SELECT or INSERT raises for ANY reason (permission, RLS, missing table), the block's transaction is rolled back, a warning naming the block and the error is logged, and the block is re-run with `gated=False` in a fresh transaction. Implemented by splitting `_run_once_startup_migration` into `_run_gated_startup_migration` (raises a new `_MarkerAccessError` for marker-access failures only) and `_run_ungated_startup_migration`. A failure inside the block's own `body` is deliberately NOT retried ungated — that would just run the same failing statement twice; it stays non-fatal and unmarked, so the next boot retries it as before. Failing to TAKE the lock is classed as a marker-access failure too: that is the guard machinery failing, not the block. `_ensure_migration_markers`' docstring now records that the app role must OWN `migration_markers` — RLS is enabled in prod with no policies, an owner bypasses it and a non-owner does not, so a non-owning role would see an empty SELECT (indistinguishable from "not applied yet") and a denied INSERT — and points at the every-boot fallback warning as the symptom to look for.
  **Tests — 4 new (12 → 16 in the file).** (a) A real two-connection concurrency test: two threads, two independent connections dispatched through a per-thread pool because `main.db_pool` is a module global, released together on a `threading.Barrier`, against a schema with no markers AND no `migration_markers` table. Both boots complete without error, the marker table exists once, each of the eight markers exists exactly once, and every DDL statement appears exactly once across BOTH statement logs — compared against a `Counter` baseline taken from a single uncontended boot. (b) Three marker-denied tests, each running the whole sequence as a non-superuser role that owns the schema and every table the blocks touch, while `migration_markers` is owned by someone else: SELECT-denied (INSERT granted, SELECT not), INSERT-denied (SELECT granted, INSERT not), and RLS-denied (both granted, RLS enabled with no policies). In each case the boot succeeds, every gated block warns by name and names the cause, every expected column and relation is applied via the ungated path, and `migration_markers` stays empty. (c) The existing second-boot statement-log test is untouched and still green.
- **Mutation-verified, both reverted:** removing the advisory lock (`_lock_startup_migrations` → no-op) fails the concurrency test on 3 of 3 runs, with every ALTER TABLE and CREATE doubled across the two logs, and fails nothing else; removing the ungated retry (treat `_MarkerAccessError` like any other failure) fails all three marker-denied tests on the substantive assertion — the DDL was never applied — and fails nothing else. `git diff` carries neither mutation.
- **Test count:** 722 passed, 0 failed (was 718). Run in three foreground chunks (231 / 226 / 265) and once whole.
- **Why:** The Codex review of PR #41 found the guard's concurrency argument load-bearing but incomplete. `IF NOT EXISTS` + `ON CONFLICT DO NOTHING` makes a racing boot harmless, but it does not stop it: both instances still issue every ALTER TABLE, so the ACCESS EXCLUSIVE cost the guard exists to remove comes straight back on exactly the event — a deploy or autoscale starting several processes at once — that motivated it. And the marker read was trusted absolutely: an unreadable or empty-by-RLS `migration_markers` read as "already applied", which would have silently stopped applying schema on a database whose bookkeeping table was merely misconfigured. Falling back to today's every-boot behaviour with a visible warning is the strictly safer failure.

---

## 2026-09-10 23:55 — Post-051 cleanup: startup run-once guard, released_by attribution, test rot, row 131 DEPLOYED
- **File(s) changed:** `main.py`, `tests/test_startup_migrations.py`, `tests/test_released_by_attribution.py`, `tests/test_recent_ledger.py`, `tests/test_sales_order_state_model.py`, `tests/test_ship_order_service_line.py`, `FACTORY_LEDGER_CHANGELOG.md`, `CHANGE_LOG.md`
- **What changed:** Four items on branch `chore/post-051-cleanup` off `main` at `c4873b6`, one commit each.
  **(1) Startup run-once guard.** Every DDL-bearing in-app startup migration in `main.py` is now gated on a durable marker row in `migration_markers` (the table migration 051 introduced; already in prod). One marker per block — `startup_migration_label_type`, `_004`, `_005`, `_006`, `_008`, `_010`, `_011`, `_012`. First boot with no marker runs the DDL and inserts the marker in the SAME transaction; later boots read one row and issue no DDL. Concurrent boots are safe by construction: the DDL is all `IF NOT EXISTS` and the insert is `ON CONFLICT DO NOTHING`. A block that FAILS writes no marker, so it is retried rather than skipped forever; and if `migration_markers` cannot be established the blocks run ungated, exactly as before. The Migration 007 status sweep, the 051 cutover reconciliation and Migration 009 are deliberately NOT gated — data fixes, no ACCESS EXCLUSIVE, and they must keep running every boot. The sequence moved out of `startup()` into `_run_startup_migrations()` so tests can drive it against a recording connection; the SQL inside is unchanged statement for statement, verified mechanically against `HEAD:main.py`. 12 new tests in a throwaway schema on their own connection: first boot runs the DDL and writes one marker per block, second boot issues ZERO `ALTER TABLE` (asserted from a statement log off the connection, not inferred from the schema), the ungated sweeps still migrate rows inserted after the first boot, two boots re-seed nothing. Mutation-verified — disabling the gate fails the second-boot test.
  **(2) `released_by` attribution.** The three call sites the findings doc names now use `caller_source_tag(request)` instead of `_operator_id(_)`: `cancel_order_line` and `ship_order` (both `_sales_order_ship_plan` calls). The third — order cancel via `PATCH /sales/orders/{id}/status` — was already correct as of PR #40 and is now covered by a test. `ship_order`, `ship_order_preview`, `commit_ship_order` and `cancel_order_line` gain a `request: Request` parameter, matching `close_sales_order` / `release_sales_order_allocation`. `_operator_id()` and `verify_api_key` untouched; existing rows NOT backfilled. 8 new tests: dashboard key writes `'dashboard'`, master key writes NULL, `'legacy-shared-key'` never written, a manual release and a ship agree on the same key, plus a source-level guard. `PATCH .../lines/{line_id}/cancel` is not on the dashboard allowlist, so the master key's NULL is its only reachable case — the 403 is asserted so that stays documented rather than an untested gap. Mutation-verified.
  **(3) `test_recent_ledger` rot.** The assertion pinned `dashboard.css?v=40`, `shell-layout.css?v=3`, `dashboard.js?v=56` while index.html was at v=42 / v=5 / v=59. It now reads each version out of `dashboard/index.html` and asserts the real invariant: each asset is referenced exactly once, with a numeric `?v=` cache-buster, and the file exists on disk. This was the known pre-existing suite failure; the suite is now fully green.
  **(4) Changelog.** FACTORY row 131 marked **DEPLOYED 2026-09-10 ~22:50 UTC** (PR #40 squash-merged as `c4873b6` at 22:42:30 UTC, Railway auto-deploy). The Railway deployment id could NOT be captured: the CLI is installed (4.30.3) but neither authenticated nor linked to a project here, so the row records "Railway deployment id to be added". Row 131 also now flags that **deploy checklist step 2 is still outstanding** — `tests/schema/schema.sql` has not been re-dumped since 051 was applied (still the post-050 dump, carries none of 051's columns) and `scripts/dump_prod_schema.sh` needs the prod connection string. New rows 132 (startup guard), 133 (released_by), 134 (test rot).
- **Test count:** 718 passed, 0 failed (was 697 passed + 1 known failure). +12 startup-migration tests, +8 attribution tests, and the previously-failing `test_recent_ledger` assertion now passes.
- **Why:** Post-merge cleanup of the three follow-ups the sales-order state model work left behind — the ACCESS-EXCLUSIVE-on-every-boot that wedged the harness, the split `released_by` vocabulary, and a test that had to be hand-edited on every dashboard asset bump — plus recording that PR #40 actually shipped.

---

## 2026-09-10 23:05 — Merge main (PR #39) into feat/so-state-model; FACTORY row renumbered 130 → 131; prod backfill counts recorded
- **File(s) changed:** `CHANGE_LOG.md`, `FACTORY_LEDGER_CHANGELOG.md`
- **What changed:** Merged `origin/main` (which had just taken PR #39, design standards category 17) into `feat/so-state-model`. The only conflicts were the two changelogs, both resolved by keeping BOTH sides' entries. PR #39 and PR #40 had each claimed FACTORY row **130**; main owns 130, so this branch's row was renumbered to **131**, and the one reference to it — step 6 of the deploy checklist in the PR #40 body — was updated to match. Migration 051 was applied to prod at 2026-09-10 22:30 UTC (`migration_markers` row present), so row 131's status went from "NOT DEPLOYED; migration 051 NOT applied to prod" to "CODE NOT DEPLOYED; migration 051 APPLIED TO PROD", the "051 is unapplied everywhere except the local test DB" clause was corrected to past tense with 051 now marked FROZEN (any further schema change needs a new migration), the "DEPLOY ORDER" warning was marked SATISFIED, and the production backfill counts were recorded in row 131 and in the PR body's "Backfill counts" section: confirmed → open 20; shipped → closed/shipped_recorded 190; shipped → closed/shipped_not_recorded 6; cancelled → cancelled/other 62; unmapped 0; 278 rows total. No prod row carried status new / in_production / ready / partial_ship / invoiced. No application file was touched by this merge.
- **Why:** PR #40 was conflicting against main after PR #39 landed, and the migration it depends on has now been applied to production — so the row that said the migration was unapplied was actively misleading, and the counts it promised to record at apply time were available.

---

## 2026-09-10 21:15 — SO state model: fourth Codex review fix pass (tests + docs only)
- **File(s) changed:** `tests/test_sales_order_state_model.py`, `docs/design/so-state-model-findings.md`, `FACTORY_LEDGER_CHANGELOG.md`, `CHANGE_LOG.md`
- **What changed:** No production code: `main.py` and `migrations/` are byte-identical to the previous commit, verified with `git diff --exit-code`. `_EndpointConn.close()` now propagates instead of swallowing — a connection that will not close means a transaction is still parked, and hiding that hands the next test a wedged database. The three remaining deadlock tests were rebuilt with STAGED holders so the pre-fix ordering cannot pass them: (a) multi-product ship-vs-close now parks a holder on EACH product, starts the ship first, and asserts both writers are in the low product's blocking chain and neither in the high product's — under line-order locking the ship would be queued on the high product instead; (b) reciprocal duplicate cancels keep the single FOR SHARE holder on both order rows, with the comment now stating that the deadlock proof comes from the mutation, not from the test passing; (c) quantity-reduction-vs-exit was rebuilt onto the SAME order and line, with the exit asserted blocked DIRECTLY by the reduction which is itself blocked by the product holder — the exact chain the pre-fix code cannot produce. `_assert_no_deadlock` now also inspects a JSONResponse body, because these endpoints catch a deadlock and return it as a 500 rather than raising. `_wait_until_reaches` takes the full participant set so a chain running through the other writer is visible. All three mutations were applied one at a time to a scratch copy and fully reverted; each fails its target test. Findings doc: the false claim that every caller of the release path holds step 1 is corrected (A16 holds only the product lock), and the concurrency-proof description now states exactly what the holders do and do not establish. 116 tests in the file, suite 697 passed plus the known pre-existing test_recent_ledger failure.
- **Why:** Fourth Codex review of PR #40. The deadlock tests asserted the absence of a deadlock but their schedules were not pinned tightly enough for the pre-fix ordering to fail them, and two doc passages overstated what had been proved.

---

## 2026-09-10 19:20 — SO state model: third Codex review fix pass (1 P1, 3 P2, 1 P3)
- **File(s) changed:** `main.py`, `tests/test_sales_order_state_model.py`, `docs/design/so-state-model-findings.md`, `FACTORY_LEDGER_CHANGELOG.md`, `CHANGE_LOG.md`
- **What changed:** P1 — `add_order_lines` now takes the `sales_orders` row lock (`_lock_sales_order`, FOR NO KEY UPDATE) before the eligibility read and the inserts, and refuses with 409 ORDER_NOT_OPEN unless `state='open'`; adding a line advances the order, so state gates it like every other SO write. P2 — every no-deadlock test now FORCES the dangerous interleaving with a holder connection parked on the resource the two writers contend on at the step after their first lock (ship-vs-close: the shared product's lot rows; reciprocal duplicate-cancels: both order rows FOR SHARE, which conflicts with NKU so both queue at step 1; quantity-reduction-vs-exit: the shared product's lot rows; add-line-vs-cancel: the order row FOR SHARE), and asserts both endpoint backends are parked behind the holder before it releases. P2 — each race gives its endpoint connection a UUID `application_name` and captures the exact backend PID with `pg_backend_pid()` BEFORE invoking the endpoint; all `pg_stat_activity` / `pg_blocking_pids` assertions use that PID, never a name match. The blocked check follows the blocking CHAIN, because Postgres reports only the direct blocker and a two-deep queue has the second writer naming the first. P2 — race-fixture cleanup no longer swallows exceptions: it deletes what can be deleted, takes the one genuinely un-deletable case (an order pinned by append-only `trace_events`) as an explicit branch that closes the order instead, and lets anything else fail the test. P3 — findings doc lock section restructured into (a) explicit row locks, (b) implicit DML locks, (c) FK KEY SHARE sites, (d) inherited prelocks, (e) offline/test operations, with row 15's wording corrected (`_release_active_allocations` / `_shrink_active_allocations` are inherited-prelock functions, not independent steps). 116 tests in the file, suite 697 passed plus the known pre-existing test_recent_ledger failure.
- **Why:** Third Codex review of PR #40. The no-deadlock tests were asserting the absence of a deadlock without ever forcing the interleaving that could produce one, and backend attribution by `application_name` could in principle match the wrong connection. Also narrowed the deadlock-freedom claim: it covers sales-order write paths only, and `make` / `pack` / `reassign_lot` product ordering is now recorded as an out-of-scope follow-up with its own cross-review.

---

## 2026-09-10 17:05 — SO state model: second Codex review fix pass (lock strength, lock order, 5 should-fix + 2 nits)
- **File(s) changed:** `main.py`, `tests/test_sales_order_state_model.py`, `docs/design/so-state-model-findings.md`, `FACTORY_LEDGER_CHANGELOG.md`, `CHANGE_LOG.md`
- **What changed:** LOCK STRENGTH — every `sales_orders` and `sales_order_lines` row lock is now `FOR NO KEY UPDATE` rather than `FOR UPDATE` (exits, ship commit, `_load_allocatable_line`, `update_order_line`). Verified first that nothing in the codebase deletes or rekeys either table: no DELETE exists, the key columns are `sales_orders.id` / `sales_orders.order_number` / `sales_order_lines.id`, none is ever UPDATEd, and `order_number` is written only by the BEFORE INSERT trigger. The weaker mode still mutually excludes exit/ship/allocate but does not conflict with the FOR KEY SHARE locks that the nine inbound FK constraints take — which is what made reciprocal duplicate-cancellations (A→B and B→A) deadlock with 40P01. LOCK ORDER — order row → line rows → product/lot rows, products always ascending id; new `_lock_sales_order()`, `_lock_sales_order_lines()` and `_lock_allocation_products()` helpers express it in one place. Line mutations (`cancel_order_line`, `update_order_line`, incl. quantity reduction) now take the order row lock first. `ship_order` commit locks the order's lines after the order row and pre-acquires every shipment product ascending BEFORE either planning loop. Legacy `PATCH /status` runs the state and fulfillment guards BEFORE the MANUAL_TRANSITIONS gate, so a fully shipped open order asked to cancel gets the structured 409 ORDER_ALREADY_SHIPPED with the close suggestion instead of a generic 400. Race tests now give the endpoint connection a distinct `application_name` and assert via `pg_stat_activity` / `pg_blocking_pids()` that the endpoint's own backend is blocked by the holder AND that its waiting statement is the initial order-row SELECT — verified by mutation that removing either row lock (state check retained) fails the assertion. Migration rerun tests moved into a throwaway schema on their own connection where "empty" and "every backfilled row changed" are established and asserted. Race fixtures now delete the unreferenced customer, deactivate the product, and close any order the append-only ledger pins in place; ledger rows stay. 115 tests in the file (11 added), suite 696 passed plus the known pre-existing test_recent_ledger failure.
- **Why:** Second Codex review of PR #40 (MERGE AFTER FIXES). The earlier pass had specified `FOR UPDATE`, which was too strong: it conflicts with the FK `KEY SHARE` locks taken by allocation inserts and by `related_so_id`, creating two real deadlock paths. Owner corrected the shorthand and set the lock order explicitly.

---

## 2026-09-10 15:40 — SO state model: Codex cross-review fix pass (2 blockers, 8 should-fix)
- **File(s) changed:** `main.py`, `migrations/051_sales_order_state_model.sql`, `tests/test_sales_order_state_model.py`, `docs/design/so-state-model-findings.md`, `FACTORY_LEDGER_CHANGELOG.md`, `CHANGE_LOG.md`
- **What changed:** BLOCKERS — (1) `ship_order` commit now locks the sales_orders row (`FOR UPDATE OF so`) BEFORE eligibility checks and any product/lot lock, and refuses unless `state='open'`; the emit_trace_event fail-hard block is untouched. (2) Allocation eligibility (`_load_allocatable_line`) now requires `state='open'` under its existing order lock. Lock order is normative everywhere: sales_orders row first, then product/lot. SHOULD-FIX — exit release is order-scoped (no product-wide auto-FIFO expiry, so closing one order cannot release another's reservation); `changed_by` is stripped but never truncated and >120 chars is a 400, while `released_by` always comes from `caller_source_tag(request)`; the state_reason CHECK now carries `IS NOT NULL` per terminal branch plus a trailing `IS TRUE` so a NULL reason cannot pass; backfill idempotency moved to a new durable `migration_markers(name PK, applied_at)` table; `related_so_id` must exist, must not be self, and is rejected for reasons that cannot use it, identically in preview and commit; health info reports `unallocated_need_lb` whenever > 0 rather than only when nothing is allocated; `attribution_note` and `resulting_state_changed_by` now appear in preview responses too. POLICY — legacy `PATCH /sales/orders/{id}/status` routes `cancelled` through the cancel logic (incl. the unshipped guard) and `invoiced` through close (mirroring `'invoiced'`, not `'shipped'`), while new/confirmed/in_production/ready keep today's behaviour and never touch state; an idempotent startup sweep reconciles legacy `status='cancelled'` rows to `state='cancelled'` and deliberately leaves Open · Shipped rows alone. 46 new tests (104 in the file), including shipment-vs-cancel, allocation-vs-close and close-vs-close races on separate real DB connections. Suite 685 passed plus the known pre-existing test_recent_ledger failure.
- **Why:** Codex cross-review of PR #40. The two blockers were real lost-update windows: without the order row lock a close or cancel committing concurrently could be overtaken by a shipment or a new reservation, leaving stock moving against an order that had already left the board. Migration 051 is unapplied everywhere except the local test DB, so it was edited in place rather than superseded by a 052.

---

## 2026-09-10 14:20 — SO state model: migration 051, administrative exits, derived fulfillment + tiered health
- **File(s) changed:** `migrations/051_sales_order_state_model.sql`, `main.py`, `tests/test_sales_order_state_model.py`, `docs/design/so-state-model-findings.md`, `FACTORY_LEDGER_CHANGELOG.md`, `CHANGE_LOG.md`
- **What changed:** Migration 051 adds `state` / `state_reason` / `state_note` / `state_changed_at` / `state_changed_by` / `related_so_id` / `status_before_exit` to `sales_orders` with per-state reason CHECKs, an FK on related_so_id, an index on (state), and an idempotent backfill that uses the LEDGER (effective_status='posted') as the tiebreaker for the shipped_recorded vs shipped_not_recorded split. `sales_orders.status` and `sales_order_lines.line_status` are untouched. main.py gains three dashboard-key-allowlisted endpoints (POST /sales/orders/{id}/close | /cancel | /reopen) on the existing preview/commit body-mode pattern, a `state=` list filter, GET /sales/orders/counts (declared before /sales/orders/{order_id}), and derived `fulfillment` + `health` on both the list and detail responses. Close/cancel release the order's active reservations via the existing audited `_release_active_allocations()` path and create zero ledger rows and zero trace events. 58 new tests; full suite 639 passed plus the known pre-existing test_recent_ledger failure. No dashboard/ or openapi-gpt-v3.yaml changes.
- **Why:** `status` was doing three unrelated jobs at once — administrative intent, physical fulfillment, and "does this need attention" — so there was no way to short-close an order, say why it left the board, or mark it a duplicate. State is stored because only a human knows the intent; fulfillment and health are derived because the ledger already knows them and a stored copy would drift. The mirror onto `status` keeps all fifteen existing status readers and the dashboard correct during the deprecation window.
...
- **Files:** `FACTORY_LEDGER_CHANGELOG.md`, `CHANGE_LOG.md`, `docs/design/audit/IMPROVEMENTS-MASTER.md`, `docs/design/audit-2026-09-09-batch-5.md`; global `/Users/cns/change-log.md` updated separately.
...
- **Why:** Blocking bug from the Codex cross-review of PR #36. Same branch `fix/so-intake-followup`; PR still not merged.

---

## 2026-09-09 09:56 — SO intake follow-up: PO re-match keeps human picks, picks pull master case size, PyYAML declared (FACTORY row 127)
...
- **Why:** Owner approved deploying the rebased mobile entry screen work (2026-09-09).

---

## 2026-09-09 09:40 — Rebased PR #33 (mobile entry screen) onto post-#34/#35 main; integrated with reference-calendar disclosure and mobile nav
- **File(s) changed:** `dashboard/dashboard.css`, `dashboard/dashboard.js`, `dashboard/index.html`, `dashboard/sankey.html`, `dashboard/process-flow.html`, `dashboard/traceability.html`, `dashboard/shell-layout.css`, `tests/visual/lib/screens.mjs`, `docs/design/audit/06-browser-check.md`, `FACTORY_LEDGER_CHANGELOG.md`, `CHANGE_LOG.md`
- **What changed:** Rebased `feat/mobile-entry-screen` (6 commits) onto main `71d1674`. Conflicts in 3 commits resolved keeping both sides: order-table date cells keep main's `data-sort-value` AND pass the branch's overdue flag to `formatShipByDate`; the health badge keeps main's dialog-button + failure-details logic but writes the score through the branch's `setHealthScore()` into a `.health-score` span, with a "Health:" label span now visible at every width; both changelog histories merged (branch FACTORY row renumbered 119→126, row-28 precedent). Cache-busts set strictly above live (css v40→41, js v56→57, mini-calendar css v3→5 / js v4→6 on all four pages; `intake-logic.js?v=9` and `shell-layout.css?v=3` kept). Integration fixes: ≤520px header grid re-templated for main's shell (h1 hidden; `details.reference-calendar` takes the cal cell); shell-layout's 20px panel day-cell floor scoped to 521–768px so IMP-069's 44px compact days survive; reference panel widened to `calc(100vw - 16px)`/8px padding at ≤520px (7 day columns ≥44pt); health badge min-height 44px at ≤520px (fixes a pre-existing 30px TOUCH-003 fail present on main); visual-harness `tab()` now navigates via `.mobile-nav` when `.tab-bar` is hidden (≤768px) and S-02/S-02b open the disclosure first. Verification: 59/59 Node logic tests; `node --check` clean; targeted 390px light+dark audit (S-01/02/02b/03/24–29) — before on `origin/main`: 11 FAIL + 1 WARN (health badge + table-sort TOUCH-003, header LAYOUT-020 flake); after: 12 FAIL = 8 pre-existing table-sort TOUCH-003 + 4 LAYOUT-020 refresh-press repaint cells (the documented flaky SYS-2 set; health-badge TOUCH-003 fixed). Desktop 1440 spot-check: failure set identical to main. Full 06-browser-check matrix regenerated (1542 PASS / 198 FAIL / 52 WARN across all screens).
- **Why:** PR #33 conflicted with main after #34 (SO PO intake + design-audit A–C) and #35 (design-audit-4) merged; owner asked for a rebase keeping both sides, a 390px visual audit, and a --force-with-lease push with no merge.
...
- **Why:** Owner's live smoke of ER intake surfaced review-screen problems: the Fuzzy 43% badge read as "suggestion enabled Approve", an untouched field got labeled "manual", placeholder text sat in empty lb inputs, and the wide table forced horizontal scrolling in the ~500px modal.

---

## 2026-09-08 14:57 — Phone entry screen: IMP-069 compact calendar, phone header (IMP-038), Health label (IMP-073), nowrap identifiers (IMP-070), overdue glyph (IMP-072)

- **File(s) changed:** `dashboard/mini-calendar.js`, `dashboard/mini-calendar.css`, `dashboard/dashboard.css`, `dashboard/dashboard.js`, `dashboard/index.html`, `dashboard/sankey.html`, `dashboard/process-flow.html`, `dashboard/traceability.html`, `docs/design/audit/06-browser-check.md`, `docs/design/audit/IMPROVEMENTS-MASTER.md`, `docs/design/audit/pr-screenshots/feat-mobile-entry-screen/*.png`, `FACTORY_LEDGER_CHANGELOG.md`
- **What changed:** Branch `feat/mobile-entry-screen`, three code commits, all scoped to `@media (max-width: 520px)`. **`b8c3283`** — `mini-calendar.js` gains a compact mode below 520px (`matchMedia`): one 44px row with today's date, an open-SO-count pill and a ▾ that expands the current month inline — 44px prev/next, 44px day buttons that read their ship count into a live caption on tap; open state and selection survive the ship-count re-render; the old ≤520px CSS block (78px months, 8px digits) is replaced. **`ca9a8e4`** — `.app-header` is a four-row grid at ≤520px (title + badge / theme + Refresh + Updated / calendar / search) via `display: contents` on `.header-left` / `.header-right`; `#health-badge` holds a `Health` label span (sr-only above 520px, visible on phone) and a `.health-score` span that `refreshHealthBadge` writes into; `formatShipByDate(dateStr, overdue)` adds an `#i-alert` glyph (phone) and sr-only ", overdue" text (every width) to the weekday line; the ER reference / PO number and the supplies incoming reference get spans; `.ship-by-weekday` / `.health-badge` are `position: relative` to keep the hidden spans inside `.table-scroll`. **`731ca2a`** — the identifier `white-space: nowrap` rule moved inside the ≤520px block after a frozen-clock pixel diff showed it re-flowing the 1440px Orders table (the SO # column already wraps at its hyphen on desktop). Cache-bust `dashboard.css?v=33`, `dashboard.js?v=46`, `mini-calendar.css?v=4`, `mini-calendar.js?v=5` on all four pages. IMP-069/070/072/073 marked DONE and IMP-038 Partly done in the master list; `06-browser-check.md` regenerated; seven 390px before/after screenshots committed for the PR.
- **Why:** The 2026-09-08 phone review of production at 390px. Verification against `main` at `b216d6f`: 384-capture harness run — TOUCH-003, ACCESS-008, LAYOUT-003, LAYOUT-011 cell-identical; LAYOUT-020 ±9 cells all in the set that flips on unmodified `main` (S-16/S-17 at 390 fail 7 of 12 baseline runs on the same shift). Desktop with `Date` frozen: 252 captures, 469 differing pixels total, max channel delta 1/255 — the same anti-aliasing noise `main` shows against itself. Needs Attention now starts at y=357 of 844 with all seven chips above the fold (was y≈690). PR opened against `main`, not merged.
...
- **Why:** Round-2 independent audit left findings 3, 4, 8, 11, 12 open; owner ordered fixes with regression tests on the auditor's exact reproductions.
...
- **What changed:** Independent audit report saved verbatim to `docs/designs/er-intake-audit-1.md`, then one fix commit per finding group, each with regression tests. (1) Review state separates `suggested` from `chosen` — fuzzy matches render as pre-highlighted candidates, lb computation and Approve require an explicit product pick (new pure-logic module `dashboard/er-intake-logic.js`, node:test coverage wrapped into pytest). (2) Alias lb conversions apply only when the line's normalized unit matches the alias unit; product association still reuses. (3) Product/unit changes invalidate lb_per_unit + expected lb; per-line save_alias checkbox (auto-off on lb override); the conversion is sent only when quantity × lb/unit equals the approved lb. (4) Reference/date/supplier and all line edits persist in state; supplier re-match merges (keeps exclusions + agreeing picks); match requests sequence-numbered, stale responses discarded, Approve disabled while matching; duplicate force bound to the (supplier, reference) pair. (5) Retry status writes conditional on `status <> 'approved'` + rowcount, 409 if approved mid-call. (6 partial) Per-key rate limits: extraction 30/h, signed URLs 120/h, logged 429; session-based auth recorded as Deferred in the design doc. (7) Weight parser: numeric boundaries ('.5 LB' → 0.5), 'N x M LB' → N×M, null on ambiguity. (8) `/expected-receipts/extract` is upload-only; dashboard immediately calls `/purchase-documents/{id}/extract` with a 90 s timeout (upload 60 s); both handlers plain def (threadpool). (9) Bounded read (limit+1), magic-byte mime (client Content-Type advisory), PDF ≤ 20 pages via pypdf (new pin 5.1.0) before storage. (10) Approve takes `pg_advisory_xact_lock` on hash(supplier, normalized reference) then re-checks duplicates. (11) Finite positive weights + strict YYYY-MM-DD dates in extraction.py; endpoint-level NaN/±inf rejection on approve; `math.isfinite` in `_create_expected_receipt_core` (manual endpoint covered). (12) New `upload_failed` status (049 edited in place — still local-only — named-constraint replace, re-applied to local test DB); approve requires `status='extracted'`. Cache-busts now js v48 / css v33 / er-intake-logic v3. Verified (no changes needed): RLS/grants on the two new tables match expected_receipts (no RLS, same owner/default grants; prod parity via default privileges), bucket `purchase-documents` does not exist in prod yet and the auto-create path sets `public: false`. Suite: 466 passed + the known pre-existing `test_recent_ledger` badge failure (also fails on clean main).
- **Why:** Owner-forwarded independent audit (docs/designs/er-intake-audit-1.md) found P1 paths to unconfirmed products, wrong pounds, lost review corrections and duplicate approvals; branch is no-merge until fixed.
...
## 2026-09-08 13:20 — Systemic ACCESS-008 clusters A1, A4, A5, A7, A8 and IMP-068 — contrast rule closed, `--primary` split, light palettes for the three chart pages

- **File(s) changed:** `dashboard/dashboard.css`, `dashboard/dashboard.js`, `dashboard/interaction.css`, `dashboard/index.html`, `dashboard/sankey.html`, `dashboard/process-flow.html`, `dashboard/traceability.html`, `dashboard/scheduler/seven-wells-production-board.html`, `docs/design/audit/06-browser-check.md`, `docs/design/audit/IMPROVEMENTS-MASTER.md`, `FACTORY_LEDGER_CHANGELOG.md`
- **What changed:** Branch `fix/systemic-contrast-c`, one commit per cluster of `07-systemic-clusters.md`. **A5** `f99553b` — new `--primary-text` token (dark `#669ef8`, light `#195aea`) for links, `.tab.active` and its indicator, `.show-more-btn`, the focus rings and the three chart pages' `a` rule. **A4** `001f97b` — dark `--primary` `#3b82f6` → `#1b6df5` under white text; the four hard-coded `.site-nav-link.active` literals and process-flow's retry button take `var(--primary)`. **A1** `4578b2f` — every `opacity: 0.5–0.65` on a row of readable text (done notes, Factory-Ready orders, inactive allocations, closed receipts, done supply requests, the depleted-lots table, the two empty-state icons) replaced with `color: var(--text-muted)` on the text; `.so-ready-pill` on `--badge-green-text`; disabled controls keep their opacity and gain a dashed inset outline via `interaction.css`. **A7** `ef75588` — one `::placeholder` rule on `--text-muted`; `input[type=number]` brought under the form-field rule. **A8** `cea45fe` — `[data-theme="light"]` token blocks and a head script applying the shared `dashboard-theme` preference in `sankey.html`, `process-flow.html`, `traceability.html`; their failing dark greys moved to the nearest AA value; hard-coded banner / badge / card / danger colours tokenised; traceability's graph colours read from `--col-*` and its print block uses the light palette; the scheduler's `--amber`, `--idle`, `--ok` corrected, `--ink3` added, a `::placeholder` rule, print accents blackened. **IMP-068** `e171ed9` — `.att-label` loses `overflow-wrap: anywhere`; the attention strip is one column below 420 px. Cache-bust `dashboard.css?v=32`, `dashboard.js?v=45`, `interaction.css?v=2` (`d65bfff`). `IMPROVEMENTS-MASTER.md`: cluster index rows A1/A4/A5/A7/A8, IMP-025 / IMP-067 / IMP-068 → DONE, IMP-010 / IMP-027 / IMP-043 → partly done with what remains. Regression-guard row 116.
- **Why:** ACCESS-008 was the last open contrast rule at 208 of 384 failing captures. Values were picked by a probe over all 384 captures recording every text node's computed colour, composited backdrop and opacity (30,152 nodes), taking the nearest value on the original hue and saturation that clears 4.6:1 on every surface the token was measured on. Full re-run: ACCESS-008 **208 → 0**, TOUCH-003 / LAYOUT-003 / LAYOUT-011 cell-identical, LAYOUT-020 within its known clock drift. IMP-068 was seen on a phone against production.

---

## 2026-09-08 12:29 — Systemic TOUCH-003 clusters T1–T4, T6–T10, T12 behind one `--hit-min` token; harness measures the real hit region

- **File(s) changed:** `dashboard/dashboard.css`, `dashboard/dashboard.js`, `dashboard/mini-calendar.css`, `dashboard/index.html`, `dashboard/sankey.html`, `dashboard/process-flow.html`, `dashboard/traceability.html`, `tests/visual/lib/checks.mjs`, `docs/design/audit/06-browser-check.md`, `docs/design/audit/IMPROVEMENTS-MASTER.md`, `FACTORY_LEDGER_CHANGELOG.md`
- **What changed:** Branch `fix/systemic-touch-b`, one commit per cluster group of `07-systemic-clusters.md`. **Token** `--hit-min: 44px` declared in `dashboard.css`'s `:root` and in the three standalone pages' `:root`. **Standalone controls take it directly** (`498a530` T1/T2/T7/T8, `6bef15f` T3, `ae29903` T6, `dcf3b1a` T9/T10/T12): `min-height` on `.btn-sm`, `.btn-refresh`, `.tab`, `.supplies-subtab`, `.notes-filter-btn`, `.btn-secondary`, `.btn-back`, `.site-nav-link` (now `inline-flex`) in `dashboard.css` and its three inline copies, the traceability controls, and every `input`/`select`/`textarea` via one zero-specificity `:where()` rule; `min-width` + `min-height` on `.btn-theme`, `.btn-close`, `.site-nav-toggle`, `.graph-ctrl-btn`, `#cal-prev`/`#cal-next`; `.mini-calendar-nav` 30/26/24 px → `var(--hit-min, 44px)`. **Controls inside dense rows keep their rendered box** and carry an invisible absolutely-positioned `::before` of at least 44 × 44 centred on them — `.er-actions .btn-sm`, `.allocation-release-btn`, `.order-inventory-toggle`, `.supply-request-done-btn`, `.order-expand-toggle`, `.note-action-btn`, `.show-more-btn` — chosen over padding + negative margin because these are bordered and padding would grow the visible box; the note Edit/Delete regions are pushed outward and the Factory Ready region right of the expand toggle so no neighbouring glyph is covered. **Checkboxes/radios** (T4) stay native at 20 px with `accent-color`; the wrapping `<label>` is the 44 px region, and `dashboard.js` wraps `.order-ready-checkbox` and `.note-checkbox` in `<label class="check-hit">`, moving the row-click `stopPropagation` guard onto that label. The order-line qty/price inputs in edit mode are deliberately excluded (replaced elements cannot carry a pseudo; growing them grows the row). **Harness** (`d60fb08`): `touchTargets()` unions an absolutely-positioned `::before`/`::after` and a wrapping checkbox/radio label into the measured region from geometry, records `tag`/`cls` per failure and a `passedViaExtension` list; against unmodified `main` it moves zero cells. Re-ran `npm run test:visual`: **TOUCH-003 316 → 116 failing cells, 200 cleared, zero regressions on a per-cell diff of all five rules**; ACCESS-008, LAYOUT-003 and LAYOUT-011 cell-identical; LAYOUT-020 ±3 cells of the pre-existing clock-dependent header shift (the `Updated:` timestamp is 126–136 px wide depending on its digits). Row-density probe: 1,216 of 1,221 rows unchanged in height. Cache-bust `dashboard.css?v=31`, `dashboard.js?v=44`, `mini-calendar.css?v=3` on all four pages. IMP-011 marked **Partly done** with hashes, IMP-069 annotated, ten rows added to the "Clusters shipped from 07" index; changelog row 115.
- **Why:** TOUCH-003 was the largest open rule (316 of 384 captures) and `07-systemic-clusters.md` traced it to one missing declaration — no `min-height` anywhere in the CSS (SYS-4) — so one token referenced from the control classes is the whole fix. The scheduler (T13/T14) and the SYS-3 non-controls (T5) are excluded per the brief: the first is a plan-table layout decision, the second needs IMP-031's role/tabindex work, not a size.
...
- **What changed:** Branch `fix/systemic-tokens-a`, one commit per cluster of `07-systemic-clusters.md`. **Six token values** (`86ea5a4`, `d9d77b7`, `414f1ea`, `8674c0a`): light `--text-muted` `#64748b`→`#5b697e`; `--text-dimmed` dark `#64748b`→`#909daf` and light `#94a3b8`→`#5b697e`; dark `--danger` `#ef4444`→`#f16161`; light `--category-granola` `#a16207`→`#955b06` and `--category-graham` `#15803d`→`#147839`. Each is the nearest value, hue and saturation held, that clears 4.5:1 on **every surface the token was measured rendering on** — except `--danger`, held to a 4.6:1 floor because its nearest passing value `#f15d5d` lands at exactly 4.50 and carries no margin — the surface set came from a probe over all 384 captures (80,067 measurements), not from reading the stylesheet, which caught that `--surface-hover` `#e2e8f0` is the binding surface in every light cluster and is named nowhere in the audit. **Three flex-wrap blocks** (`163bd33`): the `@media (max-width:768px)` `.header`/`.header-right` wrap that `dashboard.css:2223-2225` already gives `index.html`, copied into the inline stylesheets of `sankey.html`, `process-flow.html`, `traceability.html`; it also switches on `mini-calendar.css:136-143`, which has been inert on those pages since it was written. **One height-keyed media query** (`70fe9a2`): `@media (max-height:600px) { .app-header { position: static } .tab-bar { top: var(--site-nav-h) } }` — the first height-keyed rule in the product. **Harness** (`e801df3`): removed the `slice(0, 12)` caps at `checks.mjs:112` and `:256`, which were dropping 2,910 of 4,968 TOUCH-003 and 1,483 of 3,418 ACCESS-008 measurements from `results.json`; verdicts were never affected, only the stored evidence. `report.mjs`'s status line no longer claims no application file was changed. Re-ran `npm run test:visual`: **ACCESS-008 288→208, LAYOUT-003/ACCESS-001 28→12, LAYOUT-011 12→0 (closed), all rules 655→546 failing cells, zero regressions** on a per-cell diff. Bumped `dashboard.css?v=29→30`. Marked **IMP-066 DONE**, IMP-010 and IMP-027 **Partly done** with hashes, added Status/Commit columns to the Band 1 table and a "Clusters shipped from 07" index.
- **Why:** `07-systemic-clusters.md` priced the browser check's 659 failures by root cause and ranked A2, O1, F1 and A3/A6/A9 as the highest yield per unit of risk — token values and two media queries, no element resized, repositioned or restyled. Two results differ from the doc's prediction and are recorded rather than smoothed over: O1 cleared 16 of a predicted 24 cells because four screens carry a second offender behind the header (the Sankey column-label row, and Traceability's hard 900 px graph — both IMP-027 proper), and A3 confirms the doc's warning that no value preserves the three-step text scale, so `--text-dimmed` is folded onto `--text-muted` in light. Every ACCESS-008 failure still standing on the six new values sits under an ancestor `opacity` of 0.5–0.65, which is IMP-067 and unreachable by any colour change.
...
- **What changed:** Added `dashboard/fetch-timeout.js`, exposing `window.FL` with `fetchWithTimeout(url, options)` (a drop-in `fetch` replacement taking an extra `options.timeoutMs`, default **15000**), `isStall(error)`, `renderStall(el, retry, asOf)` and `clearStall(el)`. It drives an `AbortController`, honours any caller-supplied `signal` alongside its own, and on timeout rejects with a distinguishable `StallError` so a hang is separable from a refusal, a 5xx, or an offline browser. Linked from `index.html`, `sankey.html`, `process-flow.html` and `traceability.html` (the scheduler makes no network calls). All **13** `fetch(` call sites in `dashboard/` now go through it — nine in `dashboard.js` (`fetchAPI`, the direct lot fetch, the note toggle, delete, update and create, `fetchSalesAPI`, `exportOrdersMatrix`, `refreshHealthBadge`), one in `mini-calendar.js`, and one each in `sankey.html`, `process-flow.html` and `traceability.html`; `grep -c 'fetch('` for a bare call now returns zero in every file. Added `showLoadError(elementId, prefix, error, retry)` to `dashboard.js` and wired it into the twelve top-level loaders: on a stall the error element becomes "Server not responding — showing data from 3:42 PM." with a working **Retry** that re-runs that loader; every other failure keeps its existing message verbatim. Sankey now distinguishes a stall from "API unreachable — showing sample data"; Process Flow names a stall immediately instead of waiting for three consecutive failures; Traceability replaces its indefinite spinner with the stall message and Retry. Added `.fl-stall` / `.fl-stall-msg` / `.fl-stall-retry` to the shared `interaction.css`. Verified against a Node server that accepts the connection and never responds: a normal request passes through, a hang rejects as `StallError` within the timeout, and a connection refusal is correctly not reported as a stall.
- **Why:** Band 0 of the design audit, IMP-005 (FEEDBACK-003 Critical / Hard rule, plus ERROR-001 and ERROR-002). No fetch anywhere in the product had a timeout, an `AbortController` or a signal, so an API that accepted a connection and then hung — the common cold-container failure — left every loading state waiting forever, and the dashboard's `.loading-indicator` is static text, which the rule treats as a freeze by definition. Audit test: "After N seconds without progress, does the UI change to explain the stall and offer a next step?" Rendering the stall **over** the last-good view rather than replacing it is the other half of the audit's suggested fix and belongs to IMP-008, a Band 1 item left untouched; the error elements this writes into sit above the content, so nothing on screen is destroyed by the stall path itself.
...
- **Why:** TRACEABILITY_DESIGN §9 step 3 — dual-write the canonical trace layer from every operational commit (migration 048 tables are live in prod but empty). Committed to branch only; not pushed, no PR, backfill (step 4) is a separate PR.
...
- **Why:** Pre-PR fix of the flagged gap: the NOT VALID format CHECK is uppercase-only but mint paths passed user input through verbatim, so a lowercase or oddly spaced code would have failed at the DB as a 500.

---

## 2026-08-31 14:25 — Migration 047 lot identity (§3.1) on branch feat/047-lot-identity
- **File(s) changed:** `migrations/047_lot_identity.sql`, `main.py`, `tests/schema/schema.sql`, `tests/test_lot_identity_047.py`, `tests/test_audit_savepoints_lotcodes.py`, `tests/test_batch1_correctness_security.py`, `tests/test_inventory_occurred_at.py`, `tests/test_sales_order_allocations.py`, `tests/test_sales_order_readiness.py`, `FACTORY_LEDGER_CHANGELOG.md`
- **What changed:** New migration 047 (TRACEABILITY_DESIGN §3.1 identity only): `lots.lot_uuid` (uuid, NOT NULL DEFAULT gen_random_uuid(), UNIQUE), tier-1 hard unique index `lots_product_code_norm_uniq` (upper + collapse/trim whitespace + strip trailing 'LOT' token, per product, excluding merged lots), and the NOT VALID `lots_code_format_chk` format CHECK (new rows only, never VALIDATEd). main.py: `find_or_create_lot` now 409s on tier-1 twins (pre-check + index UniqueViolation mapped as race backstop) and returns `lot_uuid`; new tier-2 soft `suspicious_code_similarity` warning (aggressive normalization, savepoint-guarded, never blocks) fires at lot-mint time on /receive, /make, /pack, /inventory/found and /inventory/found-with-new-product; rename gets the same tier-1 409 pre-check; `lot_uuid` added additively to receive/make/pack commit responses, found responses, rename, GET /lots/{id}, /lots/by-code and /lots/by-supplier-lot. tests/schema/schema.sql gained a pending `\ir` 047 block (remove after prod apply + re-dump). New tests/test_lot_identity_047.py (16 tests: scratch-DB migration application on fresh + merged-twin historical shapes with zero build-time violations, twin-minting per worklist shape, lot_uuid mint/uniqueness/rename-survival/response presence). Five existing test files: lot-code seeds uppercased/underscores replaced to satisfy the new format CHECK. Suite 276/276. NOT pushed; migration NOT applied to prod.
- **Why:** §3.1 two-tier normalization decision (2026-08-31, commit 6987ee0) — prevent twin lots from casing/whitespace/'Lot'-suffix variants at mint time, warn (never block) on aggressive near-twins, and give lots a rename-stable machine identity for the upcoming QR/trace work.
...
- **Why:** Michael's review decisions on the draft; offline capability is required for plant dead zones.

---

## 2026-08-31 13:25 — Traceability design doc drafted (design only, no migrations)
- **File(s) changed:** `docs/designs/TRACEABILITY_DESIGN.md` (new, untracked)
- **What changed:** Full design for a canonical trace-event layer: evaluated three architectures (lot_links edge table; unified event tables replacing the operational model; trace_events + trace_event_lots emitted atomically by existing RECEIVE/MAKE/PACK/SHIP/ADJUST commits) and recommended the third — existing tables stay the domain model, the trace layer is the canonical genealogy graph, and traceability is never entered independently. Covers: GS1 EPCIS event semantics (transformations input→output; ships/moves = same lot + role), occurred_at vs recorded_at with a generated `late_recorded` flag, lots.lot_uuid + normalized-unique/NOT VALID CHECK collision constraints (keeping the domain-correct `UNIQUE (product_id, lot_code)`), recursive-CTE backward/forward trace, QR label + `/scan/{lot_uuid}` entry design with pallet aggregation as enum-only extension, 10-check integrity report (coverage, orphans, outputs-without-inputs, shipments-without-source, phantom consumption, 3-tier mass balance), 7-step migration/backfill plan (proposed migrations 047–048), 044-allocations conflict flags (lots/merge ILC repointing, allocation-shrink ordering, ALLOCATIONS_ENFORCED, shared-checkout collision with fix/audit-insert-savepoints), and a worked oats→batch→CQ pack→Restaurant Depot ship example with a void/re-enter correction traced both directions. Notes that no findings labeled CC-1/CC-2/CC-3 exist in the repo and maps those labels onto the actual audit findings (lot-code collisions, trace incompleteness, event-time capture).
- **Why:** Requested design for recall-grade lot traceability, built on the 8/24–8/25 data-health and traceability audits.

---

## 2026-08-31 12:20 — Problem 3: lot-code sequence robust to nonnumeric suffixes (branch fix/audit-insert-savepoints, NOT pushed)
- **File(s) changed:** `main.py`, `tests/test_audit_savepoints_lotcodes.py`, `FACTORY_LEDGER_CHANGELOG.md` (row 106 covering all three fixes)
- **What changed:** New helper `next_lot_sequence(cur, like_pattern)` computes the next auto-generated lot-code suffix as SQL `MAX(substring(lot_code FROM '-(\d{1,6})$')::int)` over codes matching the prefix (filtered by `~ '-\d{1,6}$'`), replacing the old probe (lexically-highest code + Python `int()` parse) at all 5 generator sites: `generate_lot_code()` (receive), `/make` preview + commit, `/inventory/found`, `/inventory/found-with-new-product`. Fixes the 2026-08-25 audit's Risk notes §1.3: a manually entered code sharing the prefix but ending non-numerically (e.g. `26-08-25-COST-A`, which sorts above the digits in `ORDER BY lot_code DESC`) failed the parse and reset the counter to 001 — silently colliding with the existing -001 lot via `ON CONFLICT DO NOTHING`. Numeric MAX also counts correctly past -999 (lexical `'1000' < '999'` stalled the old probe), and the found-with-new-product copy — which had NO try/except and 500'd on an unparsable suffix — is now safe. Suffixes >6 digits are ignored rather than risking an int-cast error inside the caller's transaction. 6 new tests (direct helper: nonnumeric ignored, past-999, fresh/nonnumeric-only prefix starts at 1; endpoint-level: receive preview, found, make preview mint the correct next code with a nonnumeric decoy seeded); all fail pre-fix. Suite 260 passed.
- **Why:** Problem 3 of the 2026-08-25 data-entry audit — sequence counter reset on nonnumeric suffixes caused silent lot-code collisions.

---

## 2026-08-31 12:12 — Problem 2: advisory lock on /make lot-code generation (branch fix/audit-insert-savepoints, NOT pushed)
- **File(s) changed:** `main.py`, `tests/test_audit_savepoints_lotcodes.py`
- **What changed:** `/make` commit now takes `pg_advisory_xact_lock(3)` before lot-code generation, closing the race from the 2026-08-25 audit (Risk notes §1.2): `/receive` holds lock 1 and both `/inventory/found*` paths hold lock 2, but `/make` took no lock, so two concurrent make commits could read the same MAX sequence and mint identical `B{yy-mmdd}-{seq}` codes — and for the same product, `find_or_create_lot`'s `ON CONFLICT DO NOTHING` silently folded two production runs into one lot (`lot_is_new: false` the only signal). Preview is unchanged (read-only, takes no lock; preview→commit code drift was already documented). 2 new tests via a raw-connection TestClient fixture (advisory-lock blocking is invisible through the shared savepoint-proxied test connection): (a) a make commit observably WAITS in `pg_locks` while a second real connection holds lock 3, then completes after release — fails pre-fix with "never waited"; (b) two sequential same-day makes of the same product mint consecutive distinct B-codes/lots. Suite 254 passed.
- **Why:** Problem 2 of the 2026-08-25 data-entry audit — concurrent duplicate lot codes and silent same-product lot merging on `/make`.
...
## 2026-08-25 13:26 — Added inventory occurrence time separate from entry time (PR branch; NOT deployed)
- **File(s) changed:** `main.py`, `migrations/046_inventory_occurred_at.sql`, `migrations/down/046_inventory_occurred_at_down.sql`, `tests/schema/schema.sql`, `tests/test_inventory_occurred_at.py`, `tests/test_recent_ledger.py`, `openapi-gpt-v3.yaml`, `gpt-configs/schemas/openapi-floor.yaml`, `dashboard/dashboard.js`, `dashboard/dashboard.css`, `dashboard/index.html`, `CHANGE_LOG.md`, `FACTORY_LEDGER_CHANGELOG.md`, `~/change-log.md`
- **What changed:** Rebased implementation commit `7131898` onto PR #17 head `9e78fbc`. Receive, standalone ship, make, pack, adjust, both found-inventory writes, and sales-order shipment preview/commit now accept optional ISO-8601 `occurred_at` plus `backfill`. Offset-free values are America/New_York plant-local; offset-aware values are normalized to plant time. Values over 5 minutes in the future return 400; values over 14 days old require `backfill=true`, which preserves database-owned `created_at` while marking `created_at_source='api_backfill'`. Omitting `occurred_at` retains the migration-039 timestamp fallback. Migration **046** makes explicit occurrence time trigger-safe and includes a transaction-wrapped down migration; forward re-run and down/up round trip passed on the local `factory_ledger_test` database via port 5432 only. Office/Floor GPT schemas remain exactly **30/22 operations** and instruct the GPT to ask “When did this happen?” only when the operator implies the event was not just now. Dashboard Activity now shows Occurred and Entered columns, a lag chip only when the absolute difference exceeds 60 minutes, and the existing backfilled badge; cache versions CSS 26→27 / JS 37→38.
- **Why:** Production ledger rows previously defaulted occurrence time from entry-time data, so actual event time and entry lag could not be measured. Regression coverage proves in-window plant-local persistence, future rejection, 14-day rejection, explicit backfill marking, unchanged omission behavior, and trigger preservation of explicit values. Post-rebase full suite: **239 passed, 199 warnings in 3.48s**. No FIFO/allocation changes, production DDL/data writes, deployment, or merge; `trace_ingredient`, `/transactions/history`, and `dashboard/traceability.html` were untouched by this work.
