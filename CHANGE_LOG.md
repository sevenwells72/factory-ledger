...
- **Why:** Lock the v1 response contract and warning catalog before any pack-preview implementation work begins, so the `pack-commit` and (later) `reconcile-preview` endpoints share a stable shape and so the GPT, dashboard, and audit scripts can rely on stable warning codes. Spec files in `docs/specs/` give the design a canonical home that's separate from operational/runbook docs at the repo root.
