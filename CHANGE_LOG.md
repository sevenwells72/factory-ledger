...
## 2026-08-25 13:36 — Corrected migration-039 Activity provenance (PR #18; NOT deployed)
- **File(s) changed:** `main.py`, `dashboard/dashboard.js`, `dashboard/index.html`, `migrations/046_inventory_occurred_at.sql`, `migrations/down/046_inventory_occurred_at_down.sql`, `tests/test_daily_entries.py`, `tests/test_inventory_occurred_at.py`, `tests/test_recent_ledger.py`, `CHANGE_LOG.md`, `FACTORY_LEDGER_CHANGELOG.md`, `~/change-log.md`
- **What changed:** Follow-up commit `ef3e678` makes Activity and transaction lag/cutoff reads use legacy `timestamp` as the effective entered time only for `created_at_source='migration_backfill_039'`; those rows now have zero false lag and no backfill badge. The badge is rendered only for the explicit `api_backfill` marker. Migration 046 now transactionally marks only the 2026-08-17 recon set with `api_backfill` (plant-local `created_at` date 2026-08-17, operator `inv-recon-2026-08-17`, `INV-RECON-2026-08-17` note, and business date 2026-07-24 through 2026-08-14), and its down migration restores `database`. Dashboard JS cache version is 39. Office/Floor GPT schemas remain exactly **30/22 operations**.
- **Why:** Migration-039 provenance identifies a mass-stamped `created_at`, not an intentional backfill; treating it as backfilled created false badges and lag. A transaction-scoped read-only production check proved the migration-046 selector matches exactly **75** rows (IDs within 1950–2026; the two earlier-business-date recon rows are excluded) and that their current source is `database`; no production write was made. Regression coverage includes a 039 row with no badge/zero lag, the 8/17 recon reclassification plus down reversal, and a new `backfill:true` row carrying the badge marker. Migration up/down/up passed locally. Full suite: **242 passed, 200 warnings in 3.32s**. No schema-count, GPT schema, FIFO/allocation, deployment, production-write, or merge change.
