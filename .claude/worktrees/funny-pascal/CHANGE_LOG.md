...
## 2026-03-25 17:00 — Fix /receive 503, has_bom sync, product 166 rename
- **File(s) changed:** `main.py`, `migrations/032_backfill_has_bom_and_rename_product_166.sql`
- **What changed:** (1) Added `except HTTPException: raise` to /receive preview try/except so ambiguous product 400 errors propagate instead of being swallowed as 500/503. (2) Added `UPDATE products SET has_bom = true` after batch_formulas INSERT in /admin/bom/{product_id}/lines. (3) Migration 032 backfills has_bom for all products with existing BOM lines and renames product 166 from "Sweetened Flake Coconut" to "Coconut Sweetened Flake".
- **Why:** /receive preview returned 503 on ambiguous products; product 126 had stale has_bom=false despite 6 BOM lines; product 166 couldn't be found by natural "coconut sweetened flake" search.
