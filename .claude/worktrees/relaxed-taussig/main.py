...
# Background keepalive to prevent Supabase free-tier auto-pause
...
def _run_keepalive():
    ...
    # Re-schedule; if the app is shutting down, _keepalive_timer will be
    # cancelled in the shutdown handler before the next tick fires.
    _keepalive_timer = threading.Timer(240, _run_keepalive)
...
async def startup():
    global db_pool, _keepalive_timer
    if not DATABASE_URL:
    ...
    # Start background keepalive (every 4 min) to prevent Supabase auto-pause
    _keepalive_timer = threading.Timer(240, _run_keepalive)
    _keepalive_timer.daemon = True
    _keepalive_timer.start()
    logger.info("Background keepalive started (every 240s)")
