...
async def startup():
    ...
    # Migration: Add exclude_from_inventory to batch_formulas for municipal-supply ingredients
    try:
        conn = db_pool.getconn()
        try:
            with conn.cursor() as cur:
                ...
                """)
                water_flagged = cur.rowcount

                conn.commit()
                if water_flagged > 0:
                    logger.info(f"Inventory exclusion migration: flagged {water_flagged} Water formula line(s) as exclude_from_inventory")
                else:
                    logger.info("Inventory exclusion: exclude_from_inventory column up to date")
        finally:
            db_pool.putconn(conn)
    except Exception as e:
        logger.warning(f"Inventory exclusion migration warning (non-fatal): {e}")

    # Migration: Add yield_factor to products for hydration-yield products
    try:
        conn = db_pool.getconn()
        try:
            with conn.cursor() as cur:
                ...
                """)
                conn.commit()
                logger.info("Yield factor migration: yield_factor column ready")
        finally:
            db_pool.putconn(conn)
    except Exception as e:
        logger.warning(f"Yield factor migration warning (non-fatal): {e}")

    # Migration: Add supplier_lot to lots for supplier traceability
    try:
        conn = db_pool.getconn()
        try:
            with conn.cursor() as cur:
                ...
                """)
                conn.commit()
                logger.info("Supplier lot migration: supplier_lot column ready")
        finally:
            db_pool.putconn(conn)
    except Exception as e:
        logger.warning(f"Supplier lot migration warning (non-fatal): {e}")
...
class ReceiveRequest(BaseModel):
    ...
    supplier_lot: Optional[str] = None  # Supplier's lot number for traceability (e.g., Barry Callebaut lot 123168)
...
class MakeRequest(BaseModel):
    ...
    actual_weight_lb: Optional[float] = None  # Operator-confirmed actual weight (for hydration yield products)
...
def resolve_product_full(cur, product_name: str) -> dict:
    ...
    cur.execute(
        """SELECT id, name, odoo_code, default_batch_lb, default_case_weight_lb,
                  COALESCE(yield_factor, 1.0) as yield_factor
...
def get_current_inventory(
    ...
    supplier_lot: Optional[str] = Query(default=None),
        ...
        with get_transaction() as cur:
            query = """
                SELECT p.id as product_id, p.name as product_name, p.odoo_code, p.type as product_type,
                       l.id as lot_id, l.lot_code, l.supplier_lot,
            ...
            if supplier_lot:
                query += " AND LOWER(l.supplier_lot) LIKE LOWER(%s)"
                params.append(f"%{supplier_lot}%")
...
def get_inventory_lots(
    product_id: Optional[int] = Query(default=None, description="Product ID"),
    product_name: Optional[str] = Query(default=None, description="Product name (partial match)"),
    odoo_code: Optional[str] = Query(default=None, description="Product odoo_code / SKU"),
    include_empty: bool = Query(default=False, description="Include depleted lots with 0 balance"),
    limit: int = Query(default=100, ge=1, le=500),
    _: bool = Depends(verify_api_key)
):
    """Per-lot inventory breakdown for a product. Accepts product_id, product_name, or odoo_code.
    Returns all lots sorted by age (oldest first / FIFO order)."""
    try:
        with get_transaction() as cur:
            # Resolve the product
            if product_id:
                cur.execute(
                    "SELECT id, name, odoo_code, type, uom FROM products WHERE id = %s AND COALESCE(active, true) = true",
                    (product_id,)
                )
                product = cur.fetchone()
                if not product:
                ...
                cur.execute(
                    "SELECT id, name, odoo_code, type, uom FROM products WHERE LOWER(odoo_code) = LOWER(%s) AND COALESCE(active, true) = true",
                    (odoo_code,)
                )
                product = cur.fetchone()
                if not product:
                    raise HTTPException(status_code=404, detail=f"Product with SKU '{odoo_code}' not found")
            elif product_name:
                cur.execute(
                    """SELECT id, name, odoo_code, type, uom FROM products
                       WHERE (LOWER(name) LIKE LOWER(%s) OR LOWER(odoo_code) LIKE LOWER(%s))
                         AND COALESCE(active, true) = true
                       ORDER BY name LIMIT 5""",
                    (f"%{product_name}%", f"%{product_name}%")
                ...
                if len(rows) > 1:
                    suggestions = [f"{r['name']} (ID {r['id']}, SKU {r['odoo_code'] or 'none'})" for r in rows]
                    raise HTTPException(status_code=400, detail=f"Multiple products match '{product_name}': {suggestions}. Specify product_id or odoo_code.")
                product = rows[0]
            else:
                raise HTTPException(status_code=400, detail="Provide product_id, product_name, or odoo_code")

            now = get_plant_now()

            having_clause = "HAVING COALESCE(SUM(tl.quantity_lb), 0) > 0" if not include_empty else ""

            cur.execute(f"""
                SELECT l.id as lot_id, l.lot_code, l.supplier_lot,
                       COALESCE(SUM(tl.quantity_lb), 0) as quantity_on_hand,
                       l.entry_source, l.created_at as received_date
                FROM lots l
                LEFT JOIN transaction_lines tl ON tl.lot_id = l.id
            ...
            lot_list = []
            for lot in lots:
                qty = float(lot['quantity_on_hand'])
                total_on_hand += qty
                ...
                if lot['supplier_lot']:
                    lot_data["supplier_lot"] = lot['supplier_lot']

                if lot['received_date']:
                    received_dt = lot['received_date']
                    if hasattr(received_dt, 'date'):
                        lot_data["received_date"] = received_dt.strftime("%Y-%m-%d")
                        age = (now.date() - received_dt.date()) if hasattr(received_dt, 'date') else None
                        if age is not None:
                            lot_data["age_days"] = age.days
                    else:
                        lot_data["received_date"] = str(received_dt)
        ...
        logger.error(f"Get inventory lots failed: {e}")
        return JSONResponse(status_code=500, content={"error": str(e)})
...
def get_lot_by_code(lot_code: str, _: bool = Depends(verify_api_key)):
    try:
        with get_transaction() as cur:
            # Search by system lot code OR supplier lot code
            cur.execute("""
                SELECT l.id, l.lot_code, l.supplier_lot, l.product_id, p.name as product_name, p.odoo_code,
                       COALESCE(SUM(tl.quantity_lb), 0) as quantity_on_hand,
                       l.entry_source, l.found_location, l.estimated_age
                FROM lots l
                JOIN products p ON p.id = l.product_id
                LEFT JOIN transaction_lines tl ON tl.lot_id = l.id
                WHERE LOWER(l.lot_code) = LOWER(%s) OR LOWER(l.supplier_lot) = LOWER(%s)
                GROUP BY l.id, p.id
            """, (lot_code, lot_code))
            lot = cur.fetchone()
        if not lot:
...
@app.get("/lots/search")
def search_lots(
    q: Optional[str] = Query(default=None, description="Search by system lot code, supplier lot, or product name"),
    supplier_lot: Optional[str] = Query(default=None, description="Search by supplier lot number"),
    limit: int = Query(default=50, ge=1, le=200),
    _: bool = Depends(verify_api_key)
):
    """Search lots by system lot code, supplier lot number, or product name."""
    try:
        with get_transaction() as cur:
            if not q and not supplier_lot:
                raise HTTPException(status_code=400, detail="Provide q (general search) or supplier_lot parameter")

            if supplier_lot:
                cur.execute("""
                    SELECT l.id as lot_id, l.lot_code, l.supplier_lot,
                           p.id as product_id, p.name as product_name, p.odoo_code,
                           COALESCE(SUM(tl.quantity_lb), 0) as quantity_on_hand,
                           l.entry_source
                    FROM lots l
                    JOIN products p ON p.id = l.product_id
                    LEFT JOIN transaction_lines tl ON tl.lot_id = l.id
                    WHERE LOWER(l.supplier_lot) LIKE LOWER(%s)
                    GROUP BY l.id, p.id
                    ORDER BY l.id DESC
                    LIMIT %s
                """, (f"%{supplier_lot}%", limit))
            else:
                cur.execute("""
                    SELECT l.id as lot_id, l.lot_code, l.supplier_lot,
                           p.id as product_id, p.name as product_name, p.odoo_code,
                    ...
                    JOIN products p ON p.id = l.product_id
                    LEFT JOIN transaction_lines tl ON tl.lot_id = l.id
                    WHERE LOWER(l.lot_code) LIKE LOWER(%s)
                       OR LOWER(l.supplier_lot) LIKE LOWER(%s)
        ...
        logger.error(f"Search lots failed: {e}")
        return JSONResponse(status_code=500, content={"error": str(e)})
...
def receive_preview(req: ReceiveRequest, _: bool = Depends(verify_api_key)):
    try:
        with get_transaction() as cur:
            ...
            if req.supplier_lot:
                response["supplier_lot"] = req.supplier_lot
                response["preview_message"] += f" (supplier lot: {req.supplier_lot})"
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                cur.execute("""
                    INSERT INTO lots (product_id, lot_code, entry_source, supplier_lot)
                    VALUES (%s, %s, 'received', %s)
                    RETURNING id
                """, (product['id'], lot_code, req.supplier_lot))
                ...
                if req.supplier_lot:
                    response["supplier_lot"] = req.supplier_lot
                    response["message"] += f" (supplier lot: {req.supplier_lot})"
...
def make_preview(req: MakeRequest, _: bool = Depends(verify_api_key)):
    try:
        with get_transaction() as cur:
            ...
            yield_factor = float(product.get('yield_factor') or 1.0)
            expected_actual_output = round(total_output * yield_factor, 2) if yield_factor > 1.0 else None
            ...
            inventory_excluded_ingredients = []
            ...
            for ing in formula:
                ...
                # Database-level exclusion (e.g. Water — municipal supply)
                if ing['exclude_from_inventory']:
                    inventory_excluded_ingredients.append({
                        "ingredient_id": ing_id,
                        "ingredient_name": ing['ingredient_name'],
                        "needed_lb": needed,
                        "exclude_from_inventory": True,
                        "note": "Excluded from inventory tracking (municipal supply)"
                    })
                    continue
                ...
                if lot_overrides and str(ing_id) in lot_overrides:
                    ...
                    # Fetch per-lot breakdown for this ingredient
                    cur.execute("""
                        SELECT l.id, l.lot_code, l.supplier_lot,
                    ...
                    avail = sum(float(il['available']) for il in ing_lots)
                    ...
                    for il in ing_lots:
                        lot_info = {
                            ...
                            "available_lb": float(il['available'])
                        }
                        if il['supplier_lot']:
                            lot_info["supplier_lot"] = il['supplier_lot']
                        if il['created_at']:
                            lot_info["received_date"] = il['created_at'].strftime("%Y-%m-%d") if hasattr(il['created_at'], 'strftime') else str(il['created_at'])
                            if hasattr(il['created_at'], 'date'):
                                age = (get_plant_now().date() - il['created_at'].date())
                                lot_info["age_days"] = age.days
                        lot_breakdown.append(lot_info)
                    ...
                    if len(lot_breakdown) > 0:
                        ing_data["lots"] = lot_breakdown
                    ingredients_needed.append(ing_data)
            ...
            # Hydration yield info
            if yield_factor > 1.0:
                response["yield_factor"] = yield_factor
                response["expected_actual_output_lb"] = expected_actual_output
                response["yield_note"] = (
                    f"This product has a hydration yield factor of {yield_factor}. "
                    f"Standard output: {total_output} lb → Expected actual output: ~{expected_actual_output} lb. "
                    f"When committing, you may provide actual_weight_lb with the confirmed weight "
                    f"and the system will automatically adjust the lot."
                )
            ...
            if inventory_excluded_ingredients:
                response["inventory_excluded_ingredients"] = inventory_excluded_ingredients
        ...
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                for ing in formula:
                    ...
                    # Database-level exclusion — log for traceability but don't touch inventory
                    if ing['exclude_from_inventory']:
                        inventory_excluded_from_run.append({
                            "ingredient_id": ing_id,
                            "ingredient_name": ing['ingredient_name'],
                            "formula_lb": needed,
                            "exclude_from_inventory": True,
                            "note": "Not deducted — excluded from inventory tracking (municipal supply)"
                        })
                        continue
                    ...
                    if lot_overrides and str(ing_id) in lot_overrides:
                        override_code = lot_overrides[str(ing_id)]
                        cur.execute("""
                            SELECT l.id, l.lot_code, l.supplier_lot, l.created_at FROM lots l
                            WHERE l.product_id = %s AND LOWER(l.lot_code) = LOWER(%s)
                        """, (ing_id, override_code))
                        override_lot = cur.fetchone()
                    ...
                    if override_lot:
                        ...
                        consume_entry = {"lot_code": override_lot['lot_code'], "consumed_lb": needed, "override": True}
                        if override_lot.get('supplier_lot'):
                            consume_entry["supplier_lot"] = override_lot['supplier_lot']
                        if override_lot.get('created_at') and hasattr(override_lot['created_at'], 'strftime'):
                            consume_entry["received_date"] = override_lot['created_at'].strftime("%Y-%m-%d")
                        consumed.append(consume_entry)
                    else:
                        cur.execute("""
                            SELECT l.id, l.lot_code, l.supplier_lot, l.created_at,
                                   COALESCE(SUM(tl.quantity_lb), 0) as available
                            FROM lots l
                            LEFT JOIN transaction_lines tl ON tl.lot_id = l.id
                            WHERE l.product_id = %s
                        ...
                        for lot in lots:
                            ...
                            consume_entry = {"lot_code": lot['lot_code'], "consumed_lb": take}
                            if lot.get('supplier_lot'):
                                consume_entry["supplier_lot"] = lot['supplier_lot']
                            if lot.get('created_at') and hasattr(lot['created_at'], 'strftime'):
                                consume_entry["received_date"] = lot['created_at'].strftime("%Y-%m-%d")
                            consumed.append(consume_entry)
                            remaining -= take
                ...
                # Hydration yield adjustment
                yield_factor = float(product.get('yield_factor') or 1.0)
                expected_actual_output = round(total_output * yield_factor, 2) if yield_factor > 1.0 else None
                yield_adjustment_applied = None

                if req.actual_weight_lb and req.actual_weight_lb > total_output:
                    # Operator confirmed actual weight — auto-create adjustment
                    adjustment_lb = round(req.actual_weight_lb - total_output, 2)
                    cur.execute("""
                        INSERT INTO transactions (type, timestamp, notes, adjust_reason)
                        VALUES ('adjust', %s, %s, %s)
                        RETURNING id
                    """, (
                        now,
                        f"Hydration yield adjustment for {product['name']} lot {lot_code}: "
                        f"+{adjustment_lb} lb (formula {total_output} lb → actual {req.actual_weight_lb} lb)",
                        "Hydration yield adjustment"
                    ))
                    adj_txn_id = cur.fetchone()['id']

                    cur.execute("""
                        INSERT INTO transaction_lines (transaction_id, product_id, lot_id, quantity_lb)
                        VALUES (%s, %s, %s, %s)
                    """, (adj_txn_id, product['id'], output_lot_id, adjustment_lb))

                    yield_adjustment_applied = {
                        "adjustment_transaction_id": adj_txn_id,
                        "formula_output_lb": total_output,
                        "actual_weight_lb": req.actual_weight_lb,
                        "adjustment_lb": adjustment_lb,
                        "message": f"Hydration yield adjustment: +{adjustment_lb} lb applied to lot {lot_code}"
                    }
                    logger.info(f"Hydration yield adjustment: +{adjustment_lb} lb on lot {lot_code}")
                ...
                # Include yield info when relevant
                if yield_factor > 1.0:
                    response["yield_factor"] = yield_factor
                    response["expected_actual_output_lb"] = expected_actual_output
                    if not yield_adjustment_applied:
                        response["yield_confirmation_needed"] = True
                        response["yield_note"] = (
                            f"This product has a hydration yield factor of {yield_factor}. "
                            f"Expected actual weight: ~{expected_actual_output} lb. "
                            f"Use POST /make/{lot_code}/confirm-weight with the actual weight, "
                            f"or re-commit with actual_weight_lb to apply the adjustment."
                        )

                if yield_adjustment_applied:
                    response["yield_adjustment"] = yield_adjustment_applied
                    response["output_lb"] = req.actual_weight_lb
                    response["message"] += f" (adjusted to {req.actual_weight_lb} lb — hydration yield)"
                ...
                if inventory_excluded_from_run:
                    response["inventory_excluded_ingredients"] = inventory_excluded_from_run
...
@app.post("/make/{lot_code}/confirm-weight")
def confirm_weight(lot_code: str, req: ConfirmWeightRequest, _: bool = Depends(verify_api_key)):
    """Post-production weight confirmation for hydration yield products.
    Creates an adjustment transaction to bring the lot from formula weight to actual weight."""
    try:
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                # Find the lot
                cur.execute("""
                    SELECT l.id as lot_id, l.product_id, p.name as product_name,
                           COALESCE(p.yield_factor, 1.0) as yield_factor,
                           COALESCE(SUM(tl.quantity_lb), 0) as current_balance
                    FROM lots l
                    JOIN products p ON p.id = l.product_id
                    LEFT JOIN transaction_lines tl ON tl.lot_id = l.id
                    WHERE LOWER(l.lot_code) = LOWER(%s)
                      AND l.entry_source = 'production_output'
                    GROUP BY l.id, l.product_id, p.name, p.yield_factor
                """, (lot_code,))
                lot = cur.fetchone()
                ...
                if not lot:
                    raise HTTPException(
                        status_code=404,
                        detail=f"Production lot '{lot_code}' not found"
                    )

                current_balance = float(lot['current_balance'])

                if req.actual_weight_lb <= current_balance:
                    raise HTTPException(
                        status_code=400,
                        detail=(
                            f"Actual weight ({req.actual_weight_lb} lb) must be greater than "
                            f"current lot balance ({current_balance} lb). "
                            f"For downward adjustments, use /adjust/commit."
                        )
                    )

                adjustment_lb = round(req.actual_weight_lb - current_balance, 2)
                now = get_plant_now()
                ...
                """, (
                    now,
                    f"Hydration yield adjustment for {lot['product_name']} lot {lot_code}: "
                    f"+{adjustment_lb} lb (was {current_balance} lb → now {req.actual_weight_lb} lb)",
                    "Hydration yield adjustment"
                ))
                adj_txn_id = cur.fetchone()['id']
                ...
                cur.execute("""
                    INSERT INTO transaction_lines (transaction_id, product_id, lot_id, quantity_lb)
                    VALUES (%s, %s, %s, %s)
                """, (adj_txn_id, lot['product_id'], lot['lot_id'], adjustment_lb))

                conn.commit()
                logger.info(f"Hydration yield confirmed: lot {lot_code} adjusted +{adjustment_lb} lb to {req.actual_weight_lb} lb")
                ...
                return {
                    ...
                    "adjustment_transaction_id": adj_txn_id,
                    "previous_balance_lb": current_balance,
                    "actual_weight_lb": req.actual_weight_lb,
                    "adjustment_lb": adjustment_lb,
                    "yield_factor": float(lot['yield_factor']),
                    "message": (
                        f"Weight confirmed for lot {lot_code}: "
                        f"+{adjustment_lb} lb hydration yield adjustment applied "
                        f"({current_balance} lb → {req.actual_weight_lb} lb)"
                    )
                }
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Confirm weight failed: {e}")
        return JSONResponse(status_code=500, content={"error": str(e)})
        ...
        with get_transaction() as cur:
            ...
            cur.execute("""
                SELECT p.name as ingredient_name, l.lot_code as ingredient_lot,
                       l.supplier_lot as ingredient_supplier_lot,
            ...
            for ing in ingredients:
                ...
                if ing['ingredient_supplier_lot']:
                    ing_data["supplier_lot"] = ing['ingredient_supplier_lot']
...
def trace_ingredient(lot_code: str, _: bool = Depends(verify_api_key)):
    try:
        with get_transaction() as cur:
            # Search by system lot code OR supplier lot code
            cur.execute("""
                SELECT l.lot_code, l.supplier_lot, p.name as ingredient_name, l.product_id
                FROM lots l
                JOIN products p ON p.id = l.product_id
                WHERE LOWER(l.lot_code) = LOWER(%s) OR LOWER(l.supplier_lot) = LOWER(%s)
            """, (lot_code, lot_code))
            ...
            if lot['supplier_lot']:
                response["supplier_lot"] = lot['supplier_lot']
...
def get_batch_formula(batch_id: int, _: bool = Depends(verify_api_key)):
    try:
        with get_transaction() as cur:
            cur.execute("""
                SELECT id, name, default_batch_lb, COALESCE(yield_factor, 1.0) as yield_factor
                FROM products WHERE id = %s
            """, (batch_id,))
            batch = cur.fetchone()
            ...
            batch_weight = float(batch['default_batch_lb']) if batch['default_batch_lb'] else None
            yf = float(batch['yield_factor'])

            response = {
                "batch_id": batch['id'],
                "batch_name": batch['name'],
                "batch_weight_lb": batch_weight,
                "ingredients": ingredients
            }

            if yf > 1.0:
                response["yield_factor"] = yf
                if batch_weight:
                    response["expected_actual_batch_lb"] = round(batch_weight * yf, 2)
...
class BomIngredientPatch(BaseModel):
    exclude_from_inventory: bool


@app.patch("/bom/{product_id}/ingredients/{ingredient_id}")
def patch_bom_ingredient(product_id: int, ingredient_id: int, req: BomIngredientPatch, _: bool = Depends(verify_api_key)):
    """Toggle exclude_from_inventory for a specific ingredient in a product's BOM."""
    try:
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("""
                    UPDATE batch_formulas
                    SET exclude_from_inventory = %s
                    WHERE product_id = %s AND ingredient_product_id = %s
                    RETURNING product_id, ingredient_product_id, quantity_lb, exclude_from_inventory
                """, (req.exclude_from_inventory, product_id, ingredient_id))
                updated = cur.fetchone()

                if not updated:
                    raise HTTPException(
                        status_code=404,
                        detail=f"No BOM line found for product_id={product_id}, ingredient_id={ingredient_id}"
                    )

                cur.execute("SELECT name FROM products WHERE id = %s", (product_id,))
                product = cur.fetchone()
                cur.execute("SELECT name FROM products WHERE id = %s", (ingredient_id,))
                ingredient = cur.fetchone()

                conn.commit()

                action = "excluded from" if req.exclude_from_inventory else "included in"
                logger.info(f"BOM update: {ingredient['name']} {action} inventory tracking for {product['name']}")
                ...
                return {
                    ...
                    "message": f"{ingredient['name']} is now {action} inventory tracking for {product['name']}"
                }
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Patch BOM ingredient failed: {e}")
        return JSONResponse(status_code=500, content={"error": str(e)})


class BatchProductPatch(BaseModel):
    yield_factor: Optional[float] = None

    @validator('yield_factor')
    def validate_yield_factor(cls, v):
        if v is not None and (v < 1.0 or v > 2.0):
            raise ValueError("yield_factor must be between 1.0 and 2.0")
        return v


@app.patch("/bom/{product_id}")
def patch_batch_product(product_id: int, req: BatchProductPatch, _: bool = Depends(verify_api_key)):
    """Update batch product settings like yield_factor."""
    try:
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                # Verify product exists
                cur.execute("SELECT id, name, default_batch_lb FROM products WHERE id = %s", (product_id,))
                product = cur.fetchone()
                ...
                updates = []
                params = []
                if req.yield_factor is not None:
                    updates.append("yield_factor = %s")
                    params.append(req.yield_factor)

                if not updates:
                    raise HTTPException(
                        status_code=400,
                        detail="No fields to update. Provide yield_factor."
                    )

                params.append(product_id)
                cur.execute(
                    f"UPDATE products SET {', '.join(updates)} WHERE id = %s RETURNING id, name, COALESCE(yield_factor, 1.0) as yield_factor",
                    params
                )
                updated = cur.fetchone()
                conn.commit()

                yf = float(updated['yield_factor'])
                logger.info(f"Batch product update: {updated['name']} yield_factor set to {yf}")

                response = {
                    "product_id": product_id,
                    "product_name": updated['name'],
                    "yield_factor": yf,
                    "message": f"Yield factor for {updated['name']} set to {yf}"
                }

                if yf > 1.0 and product['default_batch_lb']:
                    batch_lb = float(product['default_batch_lb'])
                    response["example"] = (
                        f"1 batch at {batch_lb} lb formula weight → "
                        f"~{round(batch_lb * yf, 2)} lb expected actual output"
                    )
        ...
        logger.error(f"Patch batch product failed: {e}")
        return JSONResponse(status_code=500, content={"error": str(e)})
