...
updated on `feat/scheduling-s1` (off `origin/main` @ `8bfdd6b`).
Status: **S1 DONE (schema + endpoints, branch `feat/scheduling-s1`, not deployed). S2 NEXT.**
Parts 1–3 are the discovery draft as reviewed. **Part 4 is normative** where it
differs from them: it records the owner decisions, the units ruling, the
competing-orders rule S2 will implement, and S1 as built.
...
## Part 4 — S1 as built (2026-09-14) — NORMATIVE

Branch `feat/scheduling-s1` off `origin/main` @ `8bfdd6b`. Migration
`migrations/053_production_runs.sql`; code in `main.py` under the
"PRODUCTION RUNS — scheduling S1" banner; tests in
`tests/test_production_runs.py` plus five rows in `EXPECTED_LOCK_SEQUENCE`
(`tests/test_sales_order_state_model.py`). Nothing the dashboard or Health
reads today changes in S1.

### 4a. Owner decisions (final) — these override Parts 1–3 where they differ

1. **New tables `production_runs` + `run_coverage`.** `production_schedule` and
   `POST /schedule` are not modified, migrated, deleted, retired or depended on.
   `production_lines` and `product_line_assignments` are reused as reference
   data only.
2. **A run = one finished SKU + quantity on one day.** Coverage links run
   quantity to `sales_order_lines`. No dependence on batch→finished routing.
   The schema stays clean: no `run_kind`, `stage` or `parent_run_id` columns,
   and no constraint or endpoint contract that a future migration adding
   kitchen/batch/packing relationships would have to undo. `POST` requires
   `products.type = 'finished'` (an endpoint check, widened later, never a
   constraint).
3. **Allocations stay as optional reservations**; explicitly allocated stock is
   never available to another order. The `allocated >= remaining` gate on
   `inventory_ready` is removed in **S2**, not S1. The competing-orders rule S2
   implements is §4c. Three concepts stay separate: **Inventory** = do we
   physically have enough available; **Ready to Ship** = the floor's flag;
   **Scheduling** = is a shortage covered by planned production.
4. **Human-confirmed completion.** `GET …/evidence` reports recorded make/pack
   output for the run's product in the date window and a suggested state;
   completion is an explicit `POST …/complete`. Completing a run does **not**
   create inventory, satisfy the SO, or set Ready to Ship. **A completed run
   never masks a shortage**: once done, actual ledger data governs Health (S2);
   `done` runs do not count as coverage (§2h stands).
5. **No role scoping.** Any key that passes the post-#49 dashboard allowlist
   (shared dashboard key or any active actor key) may write; the actor is
   recorded via the #49 mechanism (`caller_source_tag`) on `created_by` /
   `updated_by` / `completed_by`.

### 4b. Units ruling — both stored, pounds canonical

Verified before the ruling: pounds are the canonical quantity everywhere a run
is compared against — `sales_order_lines.quantity_lb` /
`quantity_shipped_lb` are the only line quantities (`schema.sql:1451-1452`);
`_line_readiness()` computes remaining, coverable and shortage in lb
(`main.py:12014-12023`); the ledger is `transaction_lines.quantity_lb` and
`/pack` posts `cases × case_weight` (`main.py:8593`). Cases enter only at the
edge: order intake converts with `products.case_size_lb` or a caller-supplied
weight (`main.py:11323-11333`, `:3760-3782`); the approve path stores pounds
as "always authoritative" with unit/quantity as metadata (`:3440-3450`);
`customer_product_aliases.case_size_lb` (`schema.sql:742`) and the `/pack`
override mean the case size is not fixed per SKU; the dashboard and today-tile
derive cases back from pounds at read time (`dashboard.js:479-490`,
`main.py:17595`). Four active finished SKUs (70004, 70006, 70013, 70016 — bulk
per/lb) have no case size at all.

**Ruling (option 3):** `planned_qty_lb` is canonical and is what coverage and
S2 Health compare against `remaining_lb`. The run also records `planned_qty`,
`planned_unit` (`cases` | `lb`) and `case_size_lb_used` — the
`products.case_size_lb` the API multiplied by, exactly as order intake does —
so "1,800 cases" survives as entered and the conversion is auditable. A SKU
with no case size must be planned in `lb` (`400 CASE_WEIGHT_REQUIRED`
otherwise, the intake error code). `run_coverage` is pounds only.

### 4c. Competing-orders availability rule — for S2 to implement

**Principle.** The same unallocated inventory can never simultaneously cover
more than one order; explicit allocations take precedence.

Today each line sees the whole unallocated pool independently
(`coverable = on_hand − allocated_others`, `main.py:12022`), so two orders can
both read "covered" by the same pounds. S2 replaces that with a waterfall over
the SKU's open demand. Verified against existing semantics before adoption:
the list already sorts `requested_ship_date ASC NULLS LAST, id ASC`
(`main.py:12689`); overdue is `requested_ship_date < today AND state = 'open'`
(`:12684`, `:12706`); Health treats a past ship date as inside the critical
window (`:12485-12491`); readiness excludes cancelled lines and uses effective
pounds (`:11862`, `:12014`); `live_alloc` honours every active, unexpired
allocation regardless of source (`:11907-11914`).

**Rule.** For each finished SKU:

1. `pool = on_hand_lb − Σ active, unexpired allocations` (manual and
   `auto_fifo` alike). Each allocated line's `coverable` is its own allocation
   first — explicit allocations win before any unallocated pound is handed out.
2. Walk the SKU's competing lines in priority order and give each
   `min(pool, unallocated_need)`, decrementing `pool`. `shortage_lb` for a line
   is what it did not get.
3. **Priority:** `requested_ship_date ASC NULLS LAST`, then `sales_order_id
   ASC`, then `line_id ASC`. Overdue orders therefore go first automatically;
   undated orders go last, matching the list sort (Health's *warning* on an
   undated shortage is attention, not stock priority — an order with no
   commitment must not starve dated ones); ties break on serial ids, i.e.
   creation order, deterministically.
4. **Who competes:** lines on orders with `state = 'open'` only (closed and
   cancelled orders release their reservations and read quiet); `line_status
   <> 'cancelled'`; non-service; `remaining_lb > 0` on effective pounds.
5. Expected receipts and run coverage are **not** availability. Coverage
   answers "is the shortage scheduled" after the waterfall, never "is there
   stock".

Consequence to state on the board: later-priority orders will show a larger
shortage than today for the same stock. That is the intended change.

### 4d. Schema as built (migration 053)

```
production_runs
  id serial PK
  product_id        integer NOT NULL REFERENCES products(id)      -- the finished SKU
  planned_qty_lb    numeric(14,4) NOT NULL CHECK (> 0)           -- canonical
  planned_qty       numeric(14,4) CHECK (NULL or > 0)            -- as entered
  planned_unit      text CHECK (NULL or IN ('cases','lb'))
  case_size_lb_used numeric(14,4) CHECK (NULL or > 0)
  planned_date      date NOT NULL
  line_id           integer REFERENCES production_lines(id)      -- nullable label
  status            text NOT NULL DEFAULT 'planned'
                    CHECK (IN ('planned','in_progress','done','cancelled'))
  notes             text
  created_at / updated_at timestamptz NOT NULL DEFAULT clock_timestamp()
  completed_at      timestamptz
  created_by / updated_by / completed_by text                    -- caller_source_tag()
  INDEX (planned_date), INDEX (status)

run_coverage
  id serial PK
  run_id              integer NOT NULL REFERENCES production_runs(id)
  sales_order_line_id integer NOT NULL REFERENCES sales_order_lines(id)
  qty_lb              numeric(14,4) NOT NULL CHECK (> 0)
  created_at          timestamptz NOT NULL DEFAULT clock_timestamp()
  created_by          text
  UNIQUE (run_id, sales_order_line_id), INDEX (sales_order_line_id)
```

Differences from the §2f/§2l sketch, all per the owner decisions:
no `actual_transaction_id` (decision 4); no `created_at_source` and no
`ledger_enforce_created_at` trigger (not a ledger table); `run_coverage` has
its own `id` plus the unique pair rather than a composite PK; index on
`status` alone rather than `(product_id, status)`; **no `BEGIN`/`COMMIT`** in
the file (052 pattern, psql `-v ON_ERROR_STOP=1` applies it statement by
statement and the file is idempotent on rerun). `updated_at` is maintained by
the application.

### 4e. Endpoints as built — dashboard allowlist only, none in any yaml

| Method | Route | Locks | Notes |
|---|---|---|---|
| `GET` | `/production/runs?from&to&status&product` | none | Runs in `planned_date, id` order with their coverage rows and `covered_lb`. No derived Health fields (S2). |
| `POST` | `/production/runs` | none (plain INSERT; FK `KEY SHARE`) | Body `{product_id, planned_qty, planned_unit?, planned_date, line_id?, notes?}`. Product must be active, `type='finished'`, not `no_production`, not a service (`400 PRODUCT_NOT_SCHEDULABLE`). `line_id` defaults from a single `product_line_assignments` row. No inline coverage — keeps this path out of the SO lock graph. |
| `PATCH` | `/production/runs/{id}` | run row | `planned_qty` / `planned_unit` (re-converted), `planned_date`, `line_id`, `notes`, `status` ∈ {`planned`,`in_progress`} only — `done`/`cancelled` are `422 INVALID_STATUS_TRANSITION` pointing at the dedicated routes. Shrinking below summed coverage → `409 RUN_OVERCOVERED`. `409 RUN_NOT_EDITABLE` once done/cancelled. |
| `POST` | `/production/runs/{id}/cancel` | run row | Terminal. `reason` appended to notes. Coverage rows left in place (filtered at read time in S2). |
| `POST` | `/production/runs/{id}/complete` | run row | `status='done'`, `completed_at`, `completed_by`. Writes the run row and nothing else; returns the evidence summary alongside, which neither gates nor is changed by the call. |
| `GET` | `/production/runs/{id}/evidence` | none | Posted `make`/`pack` output lines for the run's product with `business_date` in `[planned_date − 1, planned_date + 1]`, through `POSTED_LINES` (a void disappears on its own). `suggested_state`: `looks_complete` (recorded ≥ planned), `partial`, `none`. Also `recorded_qty` in the run's native unit. Stores nothing. |
| `PUT` | `/production/runs/{id}/coverage` | orders ↑ → lines ↑ → run | Full replace; empty list clears. Under lock: every line's order `state='open'` (`409 ORDER_NOT_OPEN`), line not cancelled/fulfilled (`409 LINE_NOT_OPEN`), not a service line (`422`), same product as the run (`409 LINE_PRODUCT_MISMATCH`), `qty_lb ≤` the line's **effective** remaining (`409 COVERAGE_EXCEEDS_REMAINING`), `Σ qty_lb ≤ planned_qty_lb` (`409 RUN_OVERCOVERED`); run must be planned/in_progress (`409 RUN_NOT_ACTIVE`). |

Dropped from §2j for S1: `DELETE …/coverage/{line_id}` (PUT covers it, and the
allowlist guard forbids DELETE outside notes), `GET /production/board` (S4a),
`?include=materials` (S3), `candidate_actuals` (replaced by the evidence
route). `GET /sales/orders` list and detail are unchanged.

### 4f. Locks as built

Rows A26–A30 in `docs/design/so-state-model-findings.md` § "(a) Explicit row
locks acquired". `production_runs` is step **2b**: acquired strictly after any
order and line lock, never before one, and no run path reaches step 3
(products, lots, allocations). `PUT …/coverage` locks every distinct
`sales_orders` row ascending (`_lock_sales_order`, state check under the lock),
then each order's listed lines ascending (`_lock_sales_order_lines`), then the
run (`_lock_production_run`, `FOR NO KEY UPDATE`). Two coverage writers that
share an order serialize at that order's lock before either reaches a line; a
1→2→3 writer never waits on a run; hence no cycle through 2b. Pinned
mechanically in `EXPECTED_LOCK_SEQUENCE`.
