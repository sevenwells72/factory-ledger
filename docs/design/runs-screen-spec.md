...
- Product search currently returns no line assignment. Leave a new run's line optional, allowing the server's single-assignment default; display the returned line on the row and prefill subsequent edits. If product data supplies a default line, prefill it. Accept a production-line number because S1 exposes no dashboard-allowlisted line catalog.
...
Deliver a pushed PR with the before/after failure table and verified Netlify preview `/runs.html`. Blubber approval is a merge gate and must be reported honestly as pending until received. Changelog next free row is 144, NOT DEPLOYED, subject to renumbering on merge.
