...
Remove the legacy order-status selector and transition path. All order gates use authoritative state and effective fulfillment. The client offers header editing for open/unshipped orders; allocations and shipment previews require an open order. The backend header endpoint still gates legacy New/Confirmed status, which cannot be reconstructed from these independent dimensions; a rejected edit must explain the server lock in plain language. Correcting that backend mismatch is outside this dashboard-only PR. Eliminate legacy sales-order status reads everywhere in `dashboard/`, including calendar consumers. Allocation statuses, HTTP statuses, and unrelated domains are separate concepts and remain valid.
